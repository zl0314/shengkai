...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'main') {
    ...
    $order['unconfirmed'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('order_info') .
            " WHERE 1 " . order_query_sql('do_pay'));
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    /* 鍟嗗搧淇℃伅 */
    $goods['total_goods'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') .
            ' WHERE is_delete = 0 AND is_goods = 1');

    $goods['total_ticket'] = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') .
            ' WHERE is_delete = 0 AND is_ticket = 1 ');
