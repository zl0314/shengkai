...
//            $content = $smarty->fetch('str:' . $template['template_content']);
            $content = json_encode(array("to" => array($admin_email),
                "sub" => array(
                    "%user_name%" => array($admin_username),
                    "%reset_email%" => array($reset_email),
                    "%shop_name%" => array($_CFG['shop_name']),
                    "%send_date%" => array(local_date($_CFG['date_format']))
                )
            ));
            __load("sendEmail/sendemail","util");
            $send_email=new Email();
            /* 发送确认重置密码的确认邮件 */
            if ($send_email->send("send_password",$content)) {
                //提示信息
                $link[0]['text'] = $_LANG['back'];
                $link[0]['href'] = 'privilege.php?act=login';
