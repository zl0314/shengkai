...
//    print_r($combo_order);die;
    ...
    foreach ($order_hotel AS $key => $value) {
        $arr[$key]['goods_price'] = number_format($value['goods_price'], 2, '.', '');
        $arr[$key]['gp'] = number_format($value['gp'], 2, '.', '');
    }
    //取得订单下的酒店总价格
...
elseif ($action == 'voucher_img') {
    ...
    $order_info = get_order_pay_status($order_sn);
    if($order_info == 2){
        $json = new JSON();
