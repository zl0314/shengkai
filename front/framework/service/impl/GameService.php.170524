...
class GameService {
    ...
    public function get_game_info($game, $region) {
        $game_info = $this->game->get($game);
        if (empty($game_info)) {
            exit(非法数据);
        }
        return $this->game->get_all_good_for_game($game,$region);
    }
   ...
   public function get_game_name($game_id){
    ...
    //--end--
   public function get_game_sche($game, $region,$sche_id) {
       $game_info = $this->game->get($game);
       if (empty($game_info)) {
            exit(非法数据);
       }
        return $this->game->get_all_good_for_sche($game, $region,$sche_id);
   }
    //运动类别--start
