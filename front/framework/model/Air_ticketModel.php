...
class Air_ticketModel extends Model
{
    //put your code here
    public function add($data)
    {
        return $this->db->autoExecute($this->ecs->table("air_ticket"), $data, 'INSERT');
    }
    ...
    public function update($data)
    {
        $id = $data['id'];
        unset($data['id']);
        return $this->db->autoExecute($this->ecs->table("air_ticket"), $data, 'UPDATE', " id=$id");
    }
    ...
    {
        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket") . " WHERE id = '{$air_ticket_id}'";
     ...
     * @return array
     */
    public function get_air_ticket_list($air_line_id, $type)
    {
        /* 记录总数 */
        $filter = array();
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('air_ticket') . " WHERE 1 " . $where;
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket") . " WHERE 1 " . $where . " ORDER BY sort_order ASC LIMIT " . $filter['start'] . ",$filter[page_size]";
        $row = $this->db->getAll($sql);
        return array('air_ticket_list' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
    }
    ...
    public function get_all_air_ticket_list($air_line_id, $type = -1)
    {
        ...
        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket") . " WHERE 1 " . $where . " ORDER BY sort_order ASC";
        $row = $this->db->getAll($sql);
        return $row;
    }
    ...
    {
        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket");
        return $this->db->getAll($sql);
    }
    ...
    {
        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket") . " ORDER BY id";
        ...
        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket") . " ORDER BY id LIMIT 1";
    ...
    {
        //出发城市
        $sql = "SELECT distinct(from_ctiy) FROM " . $this->ecs->table("air_ticket");
        ...
        //抵达城市
        $sql = "SELECT distinct(to_ctiy) FROM " . $this->ecs->table("air_ticket");
    ...
    {
        $sql = "DELETE FROM" . $this->ecs->table("air_ticket") . " WHERE id='$id'";
    ...
    {
        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket") . " WHERE id = $air_id";
        return $this->db->getRow($sql);
    }
    ...
    public function air_info()
    {
        ...
        $sql = "SELECT asi.*,FROM_UNIXTIME(asi.add_time,'%Y-%m-%d %H:%i:%s') AS add_time, a.flight, s.space_name FROM " . $this->ecs->table("air_space_info") . " AS asi LEFT JOIN " . $this->ecs->table('air_ticket') . " AS a ON asi.air_id = a.id LEFT JOIN " . $this->ecs->table('ship_space') . " AS s ON asi.s_id = s.s_id " . $where . " LIMIT " . $filter['start'] . ",$filter[page_size]";
        $row = $this->db->getAll($sql);
    ...
    {
        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket") . " WHERE to_ctiy={$to_city} AND from_ctiy={$from_city}";
    ...
    {
        $sql = "SELECT * FROM " . $this->ecs->table("air_ticket") . " WHERE from_ctiy={$from_city}";
