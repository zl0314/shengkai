...
//    }
     ...
     * @param $day_info
     */
    public function set_order_air_plane_num($order_id, $day_info)
    {
        foreach ($day_info as $value) {
            $sql = "UPDATE " . $this->ecs->table("air_line_num") . "SET locked_num = locked_num + {$value['num']} WHERE air_line_id = '{$value['air_line_id']}' and date = '{$value['date']}'";
            $ret = $this->db->query($sql);
            __log("占用拼团库存: 订单ID: {$order_id}, AIR_LIND_ID = {$value['air_line_id']}, NUM = {$value['num']}, DATE = {$value['date']}, 状态: " . ($ret ? "成功" : "失败"));
        }
    }
}
