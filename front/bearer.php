...
if ($_REQUEST['act'] == "index") {
    ...
    if ($order_detail['user_id'] != $user_id) {
        show_message("您的订单不存在，请稍后再试！", '', '', 'warning');
    }

    if ($order_detail['pay_id'] == 9) {
        if ($order_detail['order_amount'] > $order_detail['money_paid']) {
            header("Location:check_balance.php?act=zhifu_cmbuat_done&order_sn={$order_sn}");
//            show_message("订单尚未支付完成", 'cmbut_pay_continue', 'check_balance.php?act=zhifu_cmbuat_done&order_sn='.$order_sn, 'warning');
        }
    }
...
if ($_GET['act'] == "add") {
    ...
    }elseif ($_GET['step'] == "center") {
        $smarty->assign('act', "add");
        $smarty->assign('temp', "center");
        $smarty->assign('order_sn', '');
        $smarty->assign('rec_id', $_REQUEST['rec_id']);
        $smarty->assign('post_url', "bearer.php?act=edit");
    }else{
        $order_sn = $_REQUEST['order_sn'];
...
if ($_GET['act'] == "edit") {
    if ($_POST['act'] == 'add') {
        ...
        } elseif ($_POST['temp'] == 'center') {
            show_message("新增持票人信息成功", array("返回个人中心"), array('user.php?act=sk_bearer_manage'), 'info');
        }
        show_message("新增持票人信息成功", array($_LANG['back_up_page']), array('bearer.php?act=index'), 'info');
    }
