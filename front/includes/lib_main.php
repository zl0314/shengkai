...
/**
 * 更新用户openid -- hechengbin --
 */
function update_user_info_openid(){
    if(!$_SESSION['wx_openid']){
        return false;
    }else{
        $sql = "UPDATE " .$GLOBALS['ecs']->table('users')." SET wx_open_id='".$_SESSION['wx_openid']."' WHERE user_id=".$_SESSION['user_id'];
        $GLOBALS['db']->query($sql);
    }
}
/*
 * 退出时清空用户openid
 */
function update_userinfo_openid($openid){
    $sql = "UPDATE ".$GLOBALS['ecs']->table('users')." SET wx_open_id=0 WHERE wx_open_id='".$openid."'";
    $GLOBALS['db']->query($sql);
}

/*
 * 将wx绑定数据插入表中---hechengbin
 */
function insert_user_info_wx($user_name){
    //查询微信用户信息
    ...
    //查询微信用户是否存在
    $sql = "SELECT openid FROM  " .$GLOBALS['ecs']->table('wx_bind')." WHERE openid='".$_SESSION['wx_openid']."'";
    $res = $GLOBALS['db']->getOne($sql);
    if($res){
        $sql = "DELETE FROM ".$GLOBALS['ecs']->table('wx_bind')." WHERE openid='".$_SESSION['wx_openid']."'";
//        $id = get_user_id($user_name);
//        $time = time();
//        $sql = "UPDATE ".$GLOBALS['ecs']->table('wx_bind')." SET user_id='".$data['user_id']."' AND bind_time=$time WHERE openid='".$_SESSION['wx_openid']."'";
        $result = $GLOBALS['db']->query($sql);
        if($result == true){
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table("wx_bind"), $data, 'INSERT');
        }
    }else {
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table("wx_bind"), $data, 'INSERT');
    }
...
function count_cart(){
    //先检测未登录的是购物车情况
    //同步球票
    $sql="SELECT * FROM ".$GLOBALS['ecs']->table('cart')." WHERE goods_type ='ticket' AND session_id= '".SESS_ID."'";
    $new_cart=$GLOBALS['db']->getAll($sql);
    $sql="SELECT * FROM ".$GLOBALS['ecs']->table('cart')." WHERE goods_type ='ticket' AND user_id= '".$_SESSION['user_id']."'";
    $old_cart=$GLOBALS['db']->getAll($sql);
    $update_rec=array();
    foreach($old_cart as $old){
        foreach($new_cart as $new){
            if($new['goods_id']==$old['goods_id']){
                if(($new['goods_number']+$old['goods_number'])>4){
                    $new_number=4;
                }else{
                    $new_number=$old['goods_number']+$new['goods_number'];
                }
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET".
                    " goods_number = '" .$new_number . "'".
                    " WHERE rec_id = '" .$old['rec_id'] . "'";
                $GLOBALS['db']->query($sql);
                array_push($update_rec,$new['rec_id']);
            }
        }
    }
    if(!empty($update_rec)){
        $rec_str=implode(",",$update_rec);
        $sql="DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE rec_id in(".$rec_str.")";
        $GLOBALS['db']->query($sql);
    }
    if(count($old_cart)>7){
        $sql="DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE goods_type ='ticket' AND session_id ='".SESS_ID."'";
        $GLOBALS['db']->query($sql);
    }else{

//        $limt=7-count($old_cart);
//        $sql="DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE rec_id not in (SELECT rec_id FROM ".$GLOBALS['ecs']->table('cart')." WHERE goods_type ='ticket' AND session_id ='".SESS_ID."' LIMIT {$limt})";
//
//        $GLOBALS['db']->query($sql);
    }
}

function update_cart_money(){
    if($_SESSION['user_id']>0){
        /* 更新购物车内商品和球票 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET".
            " c.goods_price = ( SELECT g.shop_price FROM".$GLOBALS['ecs']->table('goods')  ." as g WHERE g.goods_id =c.goods_id )".
            " WHERE user_id = '" .$_SESSION['user_id'] . "' AND c.goods_type in ('ticket')";
        $GLOBALS['db']->query($sql);

        /* 更新购物车内球票 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET c.goods_price = ( SELECT g.shop_price+ga.attr_price FROM".$GLOBALS['ecs']->table('goods')." as g , " .$GLOBALS['ecs']->table('goods_attr') ." as ga WHERE g.goods_id =c.goods_id AND ga.goods_attr_id=c.goods_attr_id) WHERE user_id = '" .$_SESSION['user_id'] . "' AND c.goods_type in ('goods')";

        $GLOBALS['db']->query($sql);
//        /* 更新购物车内机票信息 */
//        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET".
//            " c.goods_price = ( SELECT  s.s_price FROM".$GLOBALS['ecs']->table('air_space_info')  ." as s WHERE s.id =c.space_id )".
//            " WHERE user_id = '" .$_SESSION['user_id'] . "' AND c.goods_type in ('plane')";
        //新版机票预订
        //2017-08-21
        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET".
            " c.goods_price = ( SELECT  s.price FROM".$GLOBALS['ecs']->table('air_line')  ." as s WHERE s.id =c.air_id )".
            " WHERE user_id = '" .$_SESSION['user_id'] . "' AND c.goods_type in ('plane')";
        $GLOBALS['db']->query($sql);
        /* 更新购物车内酒店信息 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET".
            " c.goods_price = ( SELECT r.room_money  FROM".$GLOBALS['ecs']->table('room')  ." as r WHERE r.id =c.room_id )".
            " WHERE user_id = '" .$_SESSION['user_id'] . "' AND c.goods_type in ('hotel')";
        $GLOBALS['db']->query($sql);
    }else{
        /* 更新购物车内商品和球票 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET".
            " c.goods_price = ( SELECT g.shop_price FROM".$GLOBALS['ecs']->table('goods')  ." as g WHERE g.goods_id =c.goods_id )".
            " WHERE session_id = '" .SESS_ID. "' AND c.goods_type in ('ticket')";
        $GLOBALS['db']->query($sql);

        /* 更新购物车内球票 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET c.goods_price = ( SELECT g.shop_price+ga.attr_price FROM".$GLOBALS['ecs']->table('goods')." as g , " .$GLOBALS['ecs']->table('goods_attr') ." as ga WHERE g.goods_id =c.goods_id AND ga.goods_attr_id=c.goods_attr_id) WHERE session_id = '" .SESS_ID . "' AND c.goods_type in ('goods')";

        $GLOBALS['db']->query($sql);
        /* 更新购物车内机票信息 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET".
            " c.goods_price = ( SELECT  s.s_price FROM".$GLOBALS['ecs']->table('air_space_info')  ." as s WHERE s.id =c.space_id )".
            " WHERE session_id = '" .SESS_ID . "' AND c.goods_type in ('plane')";
        $GLOBALS['db']->query($sql);
        /* 更新购物车内酒店信息 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " as c SET".
            " c.goods_price = ( SELECT r.room_money  FROM".$GLOBALS['ecs']->table('room')  ." as r WHERE r.id =c.room_id )".
            " WHERE session_id = '" .SESS_ID . "' AND c.goods_type in ('hotel')";
        $GLOBALS['db']->query($sql);
    }
 ...
 * 获取用户信息
 */
function get_users_openid($openid){
    $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('users')." WHERE wx_open_id= '$openid'";
    return $GLOBALS['db']->getRow($sql);
}
/**
 * 获取当前用户信息
 */
function get_users_info_openid($openid){
    $sql  = 'SELECT u.user_id, u.email, u.wx_open_id, u.user_name, u.user_money, u.pay_points'.
        ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
 ...
 * @param   string  $link
 * @param   string  $href
 * @param   string  $type               信息类型：warning, error, info,cart
...
//    }
    ...
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->assign('login_fail', 1);
    $GLOBALS['smarty']->display('message_cat.html');

    exit;
}
//手机登陆错误提示信息
function mobile_show_login_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
//    assign_template();
...
function mobile_show_messagel($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
//    assign_template();
...
//    }
    ...
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('message_catl.html');

    exit;
}
function show_null_cart($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
    assign_template();
    $msg['content'] = $content;
    if (is_array($links) && is_array($hrefs))
...
//    }
    ...
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('message_cat.dwt');
