...
if ($_SERVER['HTTP_HOST'] == 'webshop.shankaisports.com') {
    if (strpos($_SERVER['REQUEST_URI'], 'user') || strpos($_SERVER['REQUEST_URI'], 'payment') || strpos($_SERVER['REQUEST_URI'], 'wx') || strpos($_SERVER['REQUEST_URI'], 'notify') || strpos($_SERVER['REQUEST_URI'], 'alipay')) {
        //招行支付消息通知,微信支付消息通知
    } else {
        header("Location: http://www.skstravel.com");
    }
}else{
...
//检查第三方来源代码
$spm = isset($_REQUEST['spm']) ? trim($_REQUEST['spm']) : '';
if ($spm && preg_match('/(^[\w\.\-]+)$/', $spm)) {
    $_SESSION['spm'] = $spm;
}
...
if (!defined('INIT_NO_SMARTY')) {
    ...
    $smarty->assign('css_path', 'themes/' . $_CFG['template'] . "/css");
    $smarty->assign('js_path', 'themes/' . $_CFG['template'] . "/js");
...
/* 取得当前广告start */
$time = gmtime();
$sql  = 'SELECT * FROM ' . $GLOBALS['ecs']->table('bill') . " WHERE status = 1 AND start_time <= '" . $time . "' AND end_time >= '" . $time . "' ";
$allAds = $GLOBALS['db']->GetAll($sql);
if (isset($smarty)) {
    $smarty->assign('ads_list', $allAds);
}
/* 取得当前广告end */
...
//新的导航分类
$sports_cate_list = $sportcat_list;

//根据运动类别查询赛事
foreach($sportcat_list as $k=>$v){
    $tmp_sub_cate = get_game($v['id']);
    $sportcat_list[$k]['scat'] = $tmp_sub_cate;

    $sports_cate_list[$k]['scat'] = array();
    if (!empty($tmp_sub_cate)) {
        $sports_cate_list[$k]['scat'] = array_chunk($tmp_sub_cate, round(count($tmp_sub_cate) / 2));
    }
}
if(isset($smarty)){
    $smarty->assign('sportcat_list', $sportcat_list);
    $smarty->assign('sports_cate_list', $sports_cate_list);
}
/* 获取运动类别列表 end */
//查询赛事
function get_game($scat_id){
    $sql = "SELECT id,game_name,is_type FROM " .$GLOBALS['ecs']->table('game')." WHERE scat_id = '$scat_id' and is_type = 1";
