...
//    $info  = array();
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer,rank_points,pay_points,user_money, user_rank,".
             " msn, qq, emergency_phone, office_phone, home_phone,email,mobile_phone, passwd_question, company_name,adress,passwd_answer ".
           "FROM " .$GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";

    $infos = $GLOBALS['db']->getRow($sql);
    $infos['user_name'] = addslashes($infos['user_name']);
//    $infos['address_name'] = $infos['address_name'];
//    $infos['address_id'] = $infos['address_id'];    
...
//    $_SESSION['email'] = $row['email'];    //注册SESSION
   ...
   /* if ($infos['user_rank'] > 0)
    ...
    $info['qq']          = $infos['qq'];
    $info['msn']          = $infos['msn'];
    $info['address_name'] = $infos['address_name'];
...
//付款凭证
function get_goods_sn($user_id, $num = 10, $start = 0){
        $arr    = array();
        $sql="SELECT order_sn,goods_amount,pay_time FROM 
...
n.numbers,
	(
            goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount
	) AS total_fee,
	g.rec_id as zbsp_num,
	h.id as hotel_num,
	pe.id as plane_num
FROM
	sk_order_info AS p
	...
	LEFT JOIN
	sk_order_plane as pe
	on p.order_id=pe.order_id
	LEFT JOIN
	sk_order_ticket as ot
	ON p.order_id=ot.order_id
	LEFT JOIN
	sk_goods as good
	ON ot.goods_id=good.goods_id
	LEFT JOIN
	sk_number as n
	ON good.number_id=n.id
	LEFT JOIN
	sk_color_manage as c
	ON n.color_id=c.color_id
...
p.add_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "&order_sn=" .$row['order_sn']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "&order_sn=" .$row['order_sn']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a>";
            }
        ...
        $row['order_pay_status'] = $row['pay_status'];
 ...
 * @return  boolean
 */
function save_consignee($consignee)
{
...
//    {
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            " SET address_id = '$consignee' WHERE user_id = '$_SESSION[user_id]'";
 ...
 * 修改个人信息
 */
function edit_userInfo($user_info,$user_id){   
    if (empty($user_id))
    {
        $GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);
...
//    $key = array_search($user_info['user_name'], $user_info);
//    if ($key !== false){
//    array_splice($user_info,$key,1);
//    }
