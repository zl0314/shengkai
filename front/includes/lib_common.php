 ...
 * @return array 赛程名称 
 */
function get_schename($id){
    $sql = "SELECT sche_name FROM sk_schedule  WHERE id=$id ";
 ...
 * @return array 场次名称 
 */
function get_numname($id){
    $sql = "SELECT num_name FROM sk_number  WHERE id=$id ";
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}
/**
 ...
 * @return array 赛场名称 
 */
function get_pitchname($id){
    $sql = "SELECT p.pitch_name FROM sk_pitch AS p,sk_number AS n  WHERE p.id=n.pitch_id AND n.id=$id ";
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}
