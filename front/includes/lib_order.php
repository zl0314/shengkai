...
//取得订单里的门票、门票颜色
function order_menpiao($order_id)
{
    $sql = "SELECT
        	ot.code,
g.goods_name as g_name,ga.id as gameid ,p.pitch_name,ga.game_name as game_name,g.goods_number as storage ,g.seat_logo,g.keywords,g.goods_sn,ot.*,n.*,c.color_value,c.is_color,r.region_name,c.color_name, SUM(ot.goods_number) as order_goods_number,SUM(ot.goods_price) as order_goods_price,ot.goods_number*ot.goods_price as formated_subtotal
FROM
	sk_order_ticket AS ot
LEFT JOIN sk_goods AS g ON ot.goods_id = g.goods_id
LEFT JOIN sk_number AS n ON g.number_id = n.id
LEFT JOIN sk_pitch AS p ON p.id = n.pitch_id
LEFT JOIN sk_region AS r ON p.region_id = r.region_id
LEFT JOIN sk_color_manage AS c ON n.color_id = c.color_id
LEFT JOIN sk_schedule AS se ON n.sche_id = se.id
LEFT JOIN sk_game AS ga ON g.game_id = ga.id
WHERE
...
{
    $sql = "SELECT
g.goods_name as g_name,ga.id as gameid ,p.pitch_name,ga.game_name as game_name,g.goods_number as storage ,g.seat_logo,g.keywords,g.goods_sn,oc.*,n.*,com.*,c.color_value,c.is_color,r.region_name,c.color_name, oc.goods_number,SUM(oc.goods_number) as order_goods_number,SUM(oc.goods_price) as order_goods_price
FROM
	sk_order_combo AS oc
LEFT JOIN sk_goods AS g ON oc.goods_id = g.goods_id
LEFT JOIN sk_combo AS com ON oc.combo_id = com.combo_id
LEFT JOIN sk_number AS n ON g.number_id = n.id
LEFT JOIN sk_pitch AS p ON p.id = n.pitch_id
LEFT JOIN sk_region AS r ON p.region_id = r.region_id
LEFT JOIN sk_color_manage AS c ON n.color_id = c.color_id
...
{
    $sql = "select Sum(goods_price) from sk_order_ticket where order_id=" . $order_id;
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}
function combo_menpiao_sum($combo_id)
{
    $sql = "select Sum(combo_price) from sk_combo AS co  where combo_id=" . $combo_id;
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}

//取得订单下的门票总价格
function order_goods_sum($order_id)
{
    $sql = "select Sum(goods_price) from sk_order_goods where order_id=" . $order_id;
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}

//取得订单下对应的酒店信息
function order_hotel($order_id)
{
    $sql = "
    SELECT ( ht.goods_price * ht.goods_number ) AS gp, ht.goods_price, ht.room_type, ht.people, h.hotel_name, ht.room_num, ht.goods_number, r.region_name, ht.hotel_start_date, ht.hotel_end_date
FROM sk_hotel as h, sk_region AS r, sk_order_hotel AS ht
WHERE h.id=ht.hotel_id AND h.region_id = r.region_id AND ht.order_id = {$order_id};
    ";
    $res = $GLOBALS['db']->getAll($sql);
...
//取得订单下酒店的总价格
function order_hotel_sum($order_id)
{
    $res = order_hotel($order_id);
    $money = 0;
...
//取得订单下对应的机票信息
function order_plane($order_id)
{
    $sql = "SELECT
op.air_id, rg.region_name as re1,rg2.region_name as re2,op.fly_date,op.return_fly_date,op.goods_number,op.goods_price,(op.goods_number*op.goods_price) as planesum
FROM 
	sk_order_plane AS op,
        sk_region as rg,
        sk_region as rg2
WHERE
	op.order_id ={$order_id} AND op.from_city=rg.region_id AND op.to_city=rg2.region_id";
    $res = $GLOBALS['db']->getAll($sql);
    return $res;
}

//取得订单下的机票总价格
function order_plane_sum($order_id)
{
    $sql = "select Sum(goods_price*goods_number) from sk_order_plane where order_id=" . $order_id;
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}
...
//取得订单下的联系人信息
function order_lianxiren($order_id)
{
    $sql = "select s.consignee,s.email,s.mobile,s.card_type,s.card_num from sk_order_info as p,sk_user_address as s where p.order_id=$order_id AND p.address_id=s.address_id";
...
{
    $sql = "select pay_name from sk_order_info where order_id=" . $order_id;
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}

//取得订单下的持票人信息
function order_bearer($order_id,$num = 10, $start = 0)
{
    $sql = "SELECT
	c.color_value,
	n.numbers,
...
AND t.bearer_id=b.id
AND g.number_id=n.id
AND n.color_id=c.color_id
LIMIT $start,$num";
    ...
    foreach ($res as $key => $k) {
        $cpr = array(
            'color_value' => $k['color_value'],
            'bearer_id' => $k['bearer_id'],
            'numbers' => $k['numbers'],
            'mail' => $k['mail'],
            'cn_name_street' => $k['cn_name_street'],
            'goods_name' => $k['goods_name'],
            'cn_customer_name' => $k['cn_customer_name'],
            'passport_number' => $k['passport_number'],
            'num_start' => date("Y年m月d日 H时i分", strtotime($k['num_start'])),
            'keywords' => $k['keywords'],
...
//取得订单下的持票人信息
function order_bearer_list_agent($order_id)
{
    $sql = "SELECT
	c.color_value,
	n.numbers,
 ...
 * 取得订单下的套餐持票人信息
 */
function order_combo_bearer($order_id){
    $sql = "SELECT c.color_value,n.numbers,g.goods_name,b.cn_customer_name,b.mobile,b.cn_name_street,b.mail,b.id as bearer_id,b.passport_number,n.num_start FROM
	sk_bearer_info AS b,
	sk_order_combo AS oc,
	sk_goods as g,
	sk_number as n,
    ...
    AND oc.goods_id=g.goods_id
    AND g.number_id=n.id
    AND n.color_id=c.color_id
    AND oc.bearer_id=b.id";
    return $GLOBALS['db']->getAll($sql);
}
/**
 * 取得订单商品
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    switch ($storage) {
        case 0 :
            ...
            $sql = "SELECT goods_id, SUM(send_number) AS num  FROM " . $GLOBALS['ecs']->table('order_ticket') .
                " WHERE order_id = '$order_id'  GROUP BY goods_id";
            $sql_good = "SELECT goods_id, SUM(send_number) AS num  FROM " . $GLOBALS['ecs']->table('order_goods') .
                " WHERE order_id = '$order_id'  GROUP BY goods_id";
            break;

        case 1 :

            $sql = "SELECT gk.goods_id, SUM(gk.goods_number) AS num ,g.goods_number FROM " . $GLOBALS['ecs']->table('order_ticket') . "AS  gk,"
                . $GLOBALS['ecs']->table('goods') . "AS  g"
                . " WHERE gk.order_id = $order_id AND g.goods_id=gk.goods_id   GROUP BY gk.goods_id";
            $sql_good = "SELECT gk.goods_id, SUM(gk.goods_number) AS num ,g.goods_number FROM " . $GLOBALS['ecs']->table('order_goods') . "AS  gk,"
                . $GLOBALS['ecs']->table('goods') . "AS  g"
                . " WHERE gk.order_id = $order_id AND g.goods_id=gk.goods_id   GROUP BY gk.goods_id";
    ...
    $res = $GLOBALS['db']->query($sql);
    $res_goods = $GLOBALS['db']->query($sql_good);
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        if ($is_dec) {
            change_goods_storage($row['goods_id'], $row['goods_number'] - $row['num']);
        } else {
            change_goods_storage($row['goods_id'], $row['goods_number'] + $row['num']);
        }
...
//手机版减少库存
function mobile_change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    /* 查询订单商品信息 */
    switch ($storage) {
        case 0 :
...
function change_goods_storage($good_id, $number = 0)
{
    ...
    /* 处理商品库存 */
    $sql = $sql = "UPDATE " . $GLOBALS['ecs']->table('goods') . " SET goods_number = $number WHERE goods_id =$good_id";
 ...
 * 检测库存是否足够
 */
function check_cart_goods_number()
{
    __load("CartService");
    $cart_obj = new CartService();
    $cart_info = $cart_obj->get_cart_info();
    if(empty($cart_info['ticket']['t'])){
        foreach($cart_info['combo'] as $combo) {
//        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('combo') . " WHERE combo_id=" . $combo;
//        $combo_info = $GLOBALS['db']->getRow($sql);
//        foreach ($combo['combo_info'] as $k) {
            $json_res = json_decode($combo['combo_info']['combo_tickets'], true);
            foreach ($json_res['default'] as $value) {
                ...
                if ($goods_number < $combo['goods_number']) {
                    return $combo['goods_number'];
                }
            }
//        }
        }
        ...
        foreach ($cart_info['ticket']['t'] as $ticker) {
            ...
            foreach($cart_info['combo'] as $combo) {
                ...
                foreach ($json_res['default'] as $value) {
                    ...
                    $goods_id = $id[1];
                    $sql = "SELECT goods_number FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id=" . $goods_id;
                    $combo_goods_number = $GLOBALS['db']->getOne($sql);
                    if($ticker['goods_id'] == $goods_id){
                        $number = $goods_number + $combo_goods_number;
                        if($combo_goods_number < $number){
                            return $number;
                        }
                    }
                }
...
//        $sql = "SELECT goods_number FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id=" . $ticker['goods_id'];
//        $goods_number = $GLOBALS['db']->getOne($sql);
//        if ($goods_number < $ticker['goods_number']) {
//            return $ticker['goods_name'];
//        }
//    }
//    foreach ($cart_info['goods'] as $goods) {
//        $sql = "SELECT product_number FROM " . $GLOBALS['ecs']->table('products') . " WHERE goods_id=" . $goods['goods_id'] . " AND goods_attr=" . $goods['goods_attr_id'];
//        $goods_number = $GLOBALS['db']->getOne($sql);
