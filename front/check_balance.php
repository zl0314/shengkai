...
//    print_r($cart_info['plane']);die;
    ...
    //如果使用积分抵扣
    $use_credits = isset($_REQUEST['use_credits']) ? intval($_REQUEST['use_credits']) : 0;
    if ($use_credits) {
        //如果积分抵扣金额大于或等于订单金额,则实际支付0
        if ($credits_discount <= $cart_money['all']) {
            $cart_money['all'] = $cart_money['all'] - $credits_discount;
        } else {
            $cart_money['all'] = 0;
        }
    }
    ...
    //优惠券信息
    $coupon_info = array();
    $available_coupon_list = array();
    //如果使用了优惠券
    $user_coupon_id = isset($_REQUEST['user_coupon_id']) ? intval($_REQUEST['user_coupon_id']) : 0;
    if ($user_coupon_id) {
        //取得用户所有的优惠券
        //检查优惠券是否有效
        $user_coupon = $userCouponService->get_by_user_coupon_id($user_coupon_id);
        if (!empty($user_coupon)) {
            //如果优惠券未使用使用或并且未过期
            if ($user_coupon['status'] == 0 && strtotime($user_coupon['end_time']) > time()) {
                //检查该优惠券活动的信息
                __load("CouponClusterService");
                $couponClusterService = new CouponClusterService();
                $coupon_cluster = $couponClusterService->get_coupon_cluster_by_id($user_coupon['coupon_cluster_id']);
                if ($coupon_cluster['min_amount'] <= $cart_money['all']) {
                    $coupon_fee = $coupon_cluster['amount'];
                }
            }
        }
    }
    $available_coupon_list = $userCouponService->get_user_order_available_coupon_list(array(
        'order_amount' => $cart_money['all'],
        'user_id' => $user_id,
    ));
    if ($coupon_fee > 0) {
        //优惠金额不能大于或等于订单金额
        if ($cart_money['all'] >= $coupon_fee) {
            $cart_money['all'] = $cart_money['all'] - $coupon_fee;
        } else {
            $user_coupon_id = 0;
        }
    }
    ...
    $smarty->assign('available_coupon_list', $available_coupon_list);
    $smarty->assign('available_coupon_list_num', count($available_coupon_list));
    $smarty->assign('coupon_fee', $coupon_fee);
    $smarty->assign('user_coupon_id', $user_coupon_id);
    $smarty->assign('use_credits', $use_credits);
    $smarty->assign("cart_money", $cart_money);
...
//    $order['consignee'] = $_POST['consignee'];
        ...
        if ($use_credits) {
            ...
            $order['credits_paid'] = $credits_discount;
            $order['goods_amount'] = $cart_money['all'];
            $order['order_amount'] = $cart_money['all'];
            $credits_num = $credits_discount * $_CFG['credits_exchange'];
        }
        ...
        $order['coupon_paid'] = $coupon_fee;
        $order['goods_amount'] = $cart_money['all'];
        $order['order_amount'] = $cart_money['all'];
        $order['coupon_id'] = intval($coupon_id);
        ...
        if ($order['credits_paid'] > 0) {
            $sql = "UPDATE " . $GLOBALS['ecs']->table('users') . " SET credits_num = credits_num - '{$credits_num}', used_credits_num = used_credits_num + '{$credits_num}' WHERE user_id = '{$user_id}'";
        ...
        if ($coupon_id > 0) {
            //设置优惠券已使用
            $userCouponService->set_coupon_used(array(
                'coupon_id' => $user_coupon['coupon_id'],
                'order_id' => $order_id,
            ));
        }
    ...
    exit;
} elseif ($act == 'get_user_coupon_list') {
    ...
    __load("UserCouponService");
    $userCouponService = new UserCouponService();
    $user_coupon_list = $userCouponService->get_all_user_coupon_list($user_id);
    ...
    $cache_coupon_cluster = array();
    foreach ($user_coupon_list as $value1) {
        //如果优惠券过多,缓存一些信息
        if (empty($cache_coupon_cluster[$value1['coupon_cluster_id']])) {
            $coupon_cluster = $couponClusterService->get_coupon_cluster_by_id($value1['coupon_cluster_id']);
            if (empty($coupon_cluster)) {
                continue;
            }
            $cache_coupon_cluster[$value1['coupon_cluster_id']] = $coupon_cluster;
        }
        //拼装数组
        $coupon_list[] = array(
            'user_coupon_id' => $value1['user_coupon_id'],
            'coupon_name' => $cache_coupon_cluster[$value1['coupon_cluster_id']]['title'],
            'min_amount' => $cache_coupon_cluster[$value1['coupon_cluster_id']]['min_amount'],
            'amount' => $cache_coupon_cluster[$value1['coupon_cluster_id']]['amount'],
            'type_id' => $cache_coupon_cluster[$value1['coupon_cluster_id']]['type_id'],//优惠券类型 0:通用优惠券 1:单品优惠券, 默认目前只支持通用优惠券
            'status' => $value1['status'],
            'end_time' => $value1['end_time'],
            'coupon_id' => $value1['coupon_id'],
        );
    }
    $res = array(
        'code' => 200,
