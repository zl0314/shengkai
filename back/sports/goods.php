...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash') {
    ...
    $games_id = empty($_REQUEST['games_id']) ? 0 : intval($_REQUEST['games_id']);
    $num_name = empty($_REQUEST['num_name']) ? 0 : intval($_REQUEST['num_name']);
    $pitch_name = empty($_REQUEST['pitch_name']) ? 0 : intval($_REQUEST['pitch_name']);
    $region_name = empty($_REQUEST['region_name']) ? 0 : intval($_REQUEST['region_name']);
    $rank_name = empty($_REQUEST['rank']) ? 0 : intval($_REQUEST['rank']);
    $handler_list = array();
...
//    echo "<pre>";print_r($action_link);die;
...
//    $smarty->assign('rank',$goods_list['rank']);
    $smarty->assign('full_page', 1);
    __load("GameService");
    $obj_game = new GameService();
    $game_list = $obj_game->get_list();
    $smarty->assign('game_list', $game_list);
    //查询坐席等级 == hechengbin
    $rank = $obj_game->get_all_rank();
    $rank_num = count($rank);
    ...
    //he == end
    __load("PitchService");
    $obj_pitch = new PitchService();
    $pitch = $obj_pitch->get_pitch_name_list();
//    echo "<pre>";
//    print_r($pitch);die;
    $goods = $obj_pitch->get_All_Goods();
    $smarty->assign('goods',$goods);
    $smarty->assign('pitch_name',$pitch);
//    echo "<pre>";
//    print_r($goods);
    __load("NumberService");
    $obj_number = new NumberService();
    $number = $obj_number->getAllNumber();
...
elseif ($_REQUEST['act'] == 'ticketAllCount') {
    $sql = "SELECT  good.goods_name,good.goods_number,good.sum_number ,n.num_name,g.game_name FROM " . $ecs->table('goods') . " AS good," . $ecs->table('number') . " AS n," . $ecs->table('game') . " AS g WHERE good.number_id=n.id AND good.game_id=g.id AND good.is_real=1 AND good.is_ticket=1 AND good.is_on_sale=1 AND good.is_delete=0 AND good.is_shipping=0";
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('A1', '赛事');
    $objPHPExcel->getActiveSheet()->setCellValue('B1', '场次');
    $objPHPExcel->getActiveSheet()->setCellValue('C1', '商品名称');
    $objPHPExcel->getActiveSheet()->setCellValue('D1', '已售');
    $objPHPExcel->getActiveSheet()->setCellValue('E1', '剩余');
    $objPHPExcel->getActiveSheet()->setCellValue('F1', '总库存');

    foreach($allData as $key=>$data){
        $objPHPExcel->getActiveSheet()->setCellValue('A'.($key+2), $data['game_name']);
        $objPHPExcel->getActiveSheet()->setCellValue('B'.($key+2), $data['num_name']);
        $objPHPExcel->getActiveSheet()->setCellValue('C'.($key+2), $data['goods_name']);
        $objPHPExcel->getActiveSheet()->setCellValue('D'.($key+2), $data['sum_number']-$data['goods_number']);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.($key+2), $data['goods_number']);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.($key+2), $data['sum_number']);
    }
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
    ...
    if ($is_add) {
        ...
        $goods = $db->getRow($sql);
        $smarty->assign('sche_name', get_schename($goods['sche_id']));
        $smarty->assign('number_name', get_numname($goods['number_id']));
        $smarty->assign('pitch_name', get_pitchname($goods['number_id']));
        $smarty->assign('sche_id', $goods['sche_id']);
        /* 回写赛程 */
        $schedules = $db->getAll("SELECT * FROM " . $ecs->table('schedule') . " WHERE game_id ='$goods[game_id]'");
        $sche_html = '';
        foreach ($schedules as $schedule) {
            if ($schedule[id] == $goods['sche_id']) {
                $sche_html .= "<option value='$schedule[id]' selected>$schedule[sche_name]</option>";
            } else {
                $sche_html .= "<option value='$schedule[id]'>$schedule[sche_name]</option>";
            }
        }
        $smarty->assign('schedule_html', $sche_html);
        $smarty->assign('number_id', $goods['number_id']);
        /* 回写场次 */
        $number = $db->getAll("SELECT n.*, p.pitch_name FROM " . $ecs->table('number') . " AS n LEFT JOIN " . $ecs->table('pitch') . " AS p ON n.pitch_id = p.id WHERE n.sche_id='$goods[sche_id]'");

        $number_html = '';
        foreach ($number as $number_info) {
            if ($number_info['id'] == $goods['number_id']) {
                $number_html .= "<option value='$number_info[id]' selected>$number_info[num_name]--$number_info[pitch_name]</option>";
            } else {
                $number_html .= "<option value='$number_info[id]'>$number_info[num_name]--$number_info[pitch_name]</option>";
            }
        }
        $smarty->assign('number_html', $number_html);
        $smarty->assign('game_id', $goods['game_id']);
        /* 虚拟卡商品复制时, 将其库存置为0*/
        if ($is_copy && $code != '') {
            $goods['goods_number'] = 0;
        }
...
//    }
    ...
    $seat_logo = $_POST['seat_logo'];
    $seat_picture = $_POST['seat_picture'];
    $seat_text = $_POST['seat_text'];
    $ticket_business = $_POST['ticket_business'];
    ...
    $is_ticket = isset($_POST['is_ticket']) ? 1 : 0;
    $is_goods = isset($_POST['is_goods']) ? 1 : 0;
    $is_hotel = isset($_POST['is_hotel']) ? 1 : 0;
    $is_plane = isset($_POST['is_plane']) ? 1 : 0;
    $goods_number = isset($_POST['goods_number']) ? $_POST['goods_number'] : 0;
    $warn_number = isset($_POST['warn_number']) ? $_POST['warn_number'] : 0;
//    $sum_number = isset($_POST['sum_number']) ? $_POST['sum_number'] : 0;
    ...
    $game_id = empty($_POST['game_id']) ? '' : intval($_POST['game_id']);
    $number_id = empty($_POST['number_id']) ? '' : intval($_POST['number_id']);
    $sche_id = empty($_POST['sche_id']) ? '' : intval($_POST['sche_id']);


    $heat = empty($_POST['heat']) ? '' : intval($_POST['heat']);
    ...
    if ($is_insert) {
        if ($code == '') {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (rank, heat,goods_name, goods_name_style, goods_sn, " .
                "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                "is_on_sale, is_alone_sale, is_shipping, is_ticket,is_goods,is_hotel,is_plane,goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,game_id,sche_id,number_id,seat_logo,seat_picture,seat_color,seat_text,ticket_business)" .
                "VALUES ('$rank', '$heat', '$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', " .
                "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', " .
                "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number'," .
                " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, $is_ticket,$is_goods,$is_hotel,$is_plane," .
                " '$_POST[goods_desc]', '" . gmtime() . "', '" . gmtime() . "', '$goods_type', '$rank_integral', '$suppliers_id','$_POST[game_id]','$_POST[sche_id]','$_POST[number_id]','$_POST[seat_logo]','$_POST[seat_picture]','$_POST[seat_color]','$_POST[seat_text]','$_POST[ticket_business]')";
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (rank,heat,goods_name, goods_name_style, goods_sn, " .
                "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                "seller_note, goods_weight, goods_number, warn_number,warn_number, integral, give_integral, is_best, is_new, is_hot, is_real, " .
                "is_on_sale, is_alone_sale, is_shipping,is_ticket,is_goods,is_hotel,is_plane, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, suppliers_id,game_id,sche_id,number_id,seat_logo,seat_picture,seat_color,seat_text,ticket_business)" .
                "VALUES ('$rank', '$heat','$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', " .
                "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', " .
                "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number'," .
                " '$warn_number','$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping,$is_ticket,$is_goods,$is_hotel,$is_plane, " .
                " '$_POST[goods_desc]', '" . gmtime() . "', '" . gmtime() . "', '$goods_type', '$code', '$rank_integral','$_POST[game_id]','$_POST[sche_id]','$_POST[number_id]','$_POST[seat_logo]','$_POST[seat_picture]','$_POST[seat_color]','$_POST[seat_text]','$_POST[ticket_business]')";
        }
    } else {
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb, goods_img, original_img ,seat_logo,seat_picture" .
            " FROM " . $ecs->table('goods') .
            " WHERE goods_id = '$_REQUEST[goods_id]'";
        ...
        if ($seat_logo) {
            $sql .= "seat_logo = '$seat_logo', ";
        }
        if ($seat_picture) {
            $sql .= "seat_picture = '$seat_picture', ";
        }
        if ($code != '') {
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list']))) {
        ...
        if (isset($_POST['attr_id_list'])) {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id) {
                ...
                $attr_sequence = $_POST['attr_sequence_list'][$key];
                if (!empty($attr_value)) {
                    if (isset($goods_attr_list[$attr_id][$attr_value])) {
                        // 如果原来有，标记为更新
                        $goods_attr_list[$attr_id][$attr_value]['sign'] = 'update';
                        $goods_attr_list[$attr_id][$attr_value]['attr_price'] = $attr_price;
                        $goods_attr_list[$attr_id][$attr_value]['attr_sequence'] = $attr_sequence;
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list) {
            foreach ($attr_value_list as $attr_value => $info) {
                if ($info['sign'] == 'insert') {
                    $sql = "INSERT INTO " . $ecs->table('goods_attr') . " (attr_id, goods_id, attr_value, attr_price,attr_sequence)" .
                        "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]','$info[attr_sequence]')";

                } elseif ($info['sign'] == 'update') {
                    $sql = "UPDATE " . $ecs->table('goods_attr') . " SET attr_price = '$info[attr_price]',attr_sequence='$info[attr_sequence]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                } else {
...
//        1111111
        if (update_goods($product['goods_id'], 'sum_number', $product_count)) {
            //记录日志
            admin_log($product['goods_id'], 'update', 'goods');
        }
    }
...
function update_goods_stock($goods_id, $value)
{
    if ($goods_id) {
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('goods') . "
                SET goods_number = goods_number + $value,sum_number = sum_number + $value,
                    last_update = '" . gmtime() . "'
                WHERE goods_id = '$goods_id'";
