...
{ 
    ...
    $sql = "SELECT u.user_id, u.sex, u.type, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
...
//    print_r($type);die;
    ...
    if (!$users->edit_user(array('username'=>$username, 'password'=>$password, 'email'=>$email, 'gender'=>$sex, 'type'=>$type, 'bday'=>$birthday ), 1))
    {
    ...
    $other['msn'] = isset($_POST['extend_field1']) ? htmlspecialchars(trim($_POST['extend_field1'])) : '';
    $other['qq'] = isset($_POST['extend_field2']) ? htmlspecialchars(trim($_POST['extend_field2'])) : '';
//    $other['type'] = isset($_POST['type']) ? htmlspecialchars(trim($_POST['type'])) : '';
    $other['office_phone'] = isset($_POST['extend_field3']) ? htmlspecialchars(trim($_POST['extend_field3'])) : '';
    $other['home_phone'] = isset($_POST['extend_field4']) ? htmlspecialchars(trim($_POST['extend_field4'])) : '';
    $other['mobile_phone'] = isset($_POST['extend_field5']) ? htmlspecialchars(trim($_POST['extend_field5'])) : '';
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if($filter['agent'] == 1){
            $ex_where .= " AND type =" .$filter['agent'];
        }
        if($filter['agent'] == 2){
            $ex_where .= " AND type = 0 ";
        }
