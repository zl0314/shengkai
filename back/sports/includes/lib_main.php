 ...
 * @param   object  $filters    过滤条件
 */
function get_tickets_list($filter)
{
    $filter->keyword = json_str_iconv($filter->keyword);
    $where = get_where_sql($filter); // 取得过滤条件
    ...
    $sql = 'SELECT goods_id, goods_name '.
           'FROM ' . $GLOBALS['ecs']->table('goods') . $where;
    $row = $GLOBALS['db']->getAll($sql);
...
function get_where_sql($filter)
{
    ...
    if(!empty($filter->is_ticket)){
        $where .= ' AND is_ticket = '.$filter->is_ticket;
    }
 ...
 */
function game_list(){
     $sql = "SELECT * FROM" . $GLOBALS['ecs']->table("game") . "WHERE is_type=1" . "
            $where";
