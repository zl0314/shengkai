...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
//    $sql = "SELECT a.attr_id as attr_ids , a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.* ".
...
//    foreach($row as $key =>$vaule){
//        $row[$key]['attr_id']=$vaule['attr_ids'];
//        unset($row[$key]['attr_ids']);
//    }
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price,v.attr_sequence ".
        "FROM " .$GLOBALS['ecs']->table('attribute'). " AS a ".
        "LEFT JOIN " .$GLOBALS['ecs']->table('goods_attr'). " AS v ".
        "ON v.attr_id = a.attr_id AND v.goods_id = '$goods_id' ".
        "WHERE a.cat_id = " . intval($cat_id) ." OR a.cat_id = 0 ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    {
        $val['attr_id']= empty($val['attr_id'])?0:intval($val['attr_id']);
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />  排序<input type="text" name="attr_sequence_list[]" value="'. $val['attr_sequence'] . '" size="5" maxlength="10" />' :
...
//        $filter['rank']          = empty($_REQUEST['rank']) ? '' : trim($_REQUEST['rank']);
        ...
        if ($filter['brand_id'])
        {
         ...
         if(!empty($filter['game_id'])){
            $where .= "AND game_id = {$filter['game_id']}";
        }
          if (!empty($filter['number_id']))
        {
         
                $where .= " AND g.number_id = '$filter[number_id]'";       
        }
         if (!empty($filter['pitch_id']))
        {
...
//        }
        if (!empty($filter['keyword']))
        {
            $where .= " AND keywords LIKE '%" . mysql_like_quote($filter['keyword']) . "%'";
        }
        //坐席等级
        if (!empty($filter['rank']))
        {
//            $where .= " AND rank LIKE '%" . mysql_like_quote($filter['rank']) . "%'";
            $where .= " AND rank='".$filter['rank']."'";
        }

        //套餐和单票
        if(!empty($filter['ticket']) &&  $filter['ticket'] == 1){
            //查询属于套餐的球票
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('combo');
            $combo = $GLOBALS['db']->getAll($sql);
            foreach ($combo as $k=>$v){
                $combo[$k]['combo_tickets'] = json_decode($v['combo_tickets'],true);
                if(!empty($combo[$k]['combo_tickets']['default'])){
                    foreach($combo[$k]['combo_tickets']['default'] as $key=>$val){
                        $combo['goods'][] =  explode('|',$val);
                        $goods_ticket = array();
                        foreach ($combo['goods'] as $ke => $va){
                            $goods_ticket[] = $va[1];
                            $goods_ticket_id = array_unique($goods_ticket);
                        }
                    }
                }
            }
            $goods_ticket_id = implode(',',$goods_ticket_id);
            $where .=" AND g.goods_id in ($goods_ticket_id)";
        }
//        elseif(!empty($filter['ticket']) &&  $filter['ticket'] == 2){
//            $where = '';
//        }
        if (!empty($filter['region_name']))
        {
                 $where .= " AND r.region_name LIKE '%" . mysql_like_quote($filter['region_name']) . "%'";
        ...
        if(@$filter['region_name']) {
             $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods') . " AS g ," . $GLOBALS['ecs']->table('pitch') . " AS p ON p.id=g.pitch_id " .$where;
//                    " LEFT JOIN " . $GLOBALS['ecs']->table('region') . " AS r ON p.region_id=r.region_id " .$where;
        }
        if($filter['pitch_id'] != 0){
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods') . " AS g,".$GLOBALS['ecs']->table('number')." AS n, ".$GLOBALS['ecs']->table('pitch')." AS p WHERE n.id=g.number_id AND n.pitch_id=p.id AND is_delete='$is_delete' $where";
        }else {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods') . " AS g WHERE is_delete='$is_delete' $where";
        }
        if($filter['keyword']){
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('goods')." WHERE is_delete='$is_delete' $where";
        }
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        if($filter['pitch_id'] != 0){
            $joinSql = ",".$GLOBALS['ecs']->table('number')." AS n,".$GLOBALS['ecs']->table('pitch')." AS p ";
        }
        /* 分页大小 */
      
        $filter = page_and_size($filter);
        if($filter['game_id'] != 0 && $filter['number_id'] != 0 && $filter['pitch_id'] != 0){
            $sql = "SELECT goods_id, goods_name,sum_number, goods_type, goods_sn, shop_price,keywords ,is_on_sale, is_best, is_new, sort_order, goods_number, g.pitch_id,integral, " .
                " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote, is_goods, rank ".
                " FROM " . $GLOBALS['ecs']->table('goods') . " AS g ".$joinSql.
                "WHERE n.id=g.number_id AND n.pitch_id=p.id AND is_delete='$is_delete' $where " .
            ...
            $sql = "SELECT goods_id, goods_name,sum_number, goods_type, goods_sn, shop_price,keywords ,is_on_sale, is_best, is_new, sort_order, goods_number, g.pitch_id,integral, " .
                ...
                " ORDER BY $filter[sort_by] $filter[sort_order] " .
                " LIMIT " . $filter['start'] . ",$filter[page_size]";
        }elseif($filter['game_id'] != 0 && $filter['ticket'] == 2) {
            $sql = "SELECT goods_id, goods_name,sum_number, goods_type, goods_sn, shop_price,keywords ,is_on_sale, is_best, is_new, sort_order, goods_number, g.pitch_id,integral, " .
                " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote, is_goods, rank " .
                " FROM " . $GLOBALS['ecs']->table('goods') . " AS g " . $joinSql .
...
//echo $sql;die;
//                     " LEFT JOIN " . $GLOBALS['ecs']->table('pitch') . " AS p ON p.pitch_id=g.pitch_id " .
...
//    echo $sql;die;
    ...
    foreach($row as $key=>$goods){
        $row[$key]['shop_number']=$goods['sum_number']-$goods['goods_number'];
    }
