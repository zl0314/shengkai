...
elseif ($_REQUEST['act'] == 'list') {
    ...
    $number = $GLOBALS['db']->getAll("SELECT id,num_name FROM sk_number");
    $pitch_list = $GLOBALS['db']->getAll("SELECT id,pitch_name FROM sk_pitch");
    $rank = $GLOBALS['db']->getAll("SELECT DISTINCT rank FROM sk_goods");
    $rank_num = count($rank);
    ...
    $is_goods = $_REQUEST['is_goods'];
    //查看已取票订单和状态 --hechengbin
    $order_status = get_order_status();
    $smarty->assign('order_status',$order_status);
    //--hechengbin --end
    $smarty->assign('is_goods',$is_goods);
    $smarty->assign('game_list',$game_list);
    $smarty->assign('num_name',$number);
    $smarty->assign('pitch_list',$pitch_list);
    ...
    if (in_array($order_type, array(1,2))) {
        $_REQUEST['ticket'] = $order_type;
        $smarty->assign('ur_here', $_LANG['0' . $order_type . '_order_type_list']);
    }
    //订单类型
    $smarty->assign('ticket', isset($_REQUEST['ticket']) ? $_REQUEST['ticket'] : -1);
    $smarty->assign('order_type', $order_type);

    if ($order_type == 1) {
        $smarty->assign('ur_here', $_LANG['01_order_type_list']);
    } elseif ($order_type == 2) {
        $smarty->assign('ur_here', $_LANG['02_order_type_list']);
    } elseif ($order_type == 3) {
        $smarty->assign('ur_here', $_LANG['03_order_type_list']);
    } elseif ($order_type == 4) {
        $smarty->assign('ur_here', $_LANG['04_order_type_list']);
    } elseif ($order_type == 5) {
        $smarty->assign('ur_here', $_LANG['05_order_type_list']);
    }

    $order_list = order_list($order_type);
    $smarty->assign('order_list', $order_list['orders']);
    $smarty->assign('filter', $order_list['filter']);
    $smarty->assign('is_goods', $_REQUEST['is_goods']);
...
} elseif ($_REQUEST['act'] == 'bearer_list') {
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    $smarty->assign('ur_here', "持票人信息");
    $smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $_LANG['return_list']));
//    __load("BearerService");
//    $bearer_obj = new BearerService();
    $bearer_info = bearer_info_order_id($_GET['order_id']);
    foreach($bearer_info as $k => $v){
        $str = str_replace('@shengkai@','"',$v['ticket_code']);
        $bearer_info[$k]['ticket_code'] = str_replace("&shankai&","'",$str);
    }

    $bearer_list = bearer_info_page($_GET['order_id']);
    $smarty->assign('bearer_info', $bearer_info);

    $smarty->assign('full_page', 1);

    $smarty->assign('order_id', $_GET['order_id']);
    $smarty->assign('filter', $bearer_list['filter']);
    $smarty->assign('record_count', $bearer_list['record_count']);
    $smarty->assign('page_count', $bearer_list['page_count']);


    /* 显示模板 */
    assign_query_info();
    $smarty->display('bearer_list.htm');
} elseif ($_REQUEST['act'] == 'bearer_info') {
    /* 检查权限 */
    admin_priv('order_view');

    /* 模板赋值 */
    $smarty->assign('ur_here', "持票人信息");
    $smarty->assign('action_link', array('href' => 'order.php?act=bearer_list&order_id=' . $_GET['order_id'], 'text' => "返回上一页"));
    $bearer_info = bearer_info_id($_GET['bearer_id'],$_GET['order_id'],$_GET['goods_id']);
    $smarty->assign('bearer_info', $bearer_info);
    //查看订单状态
    $status_order = $GLOBALS['db']->getOne("SELECT order_status FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id=$_GET[order_id]");
    $smarty->assign('full_page', 1);
    $smarty->assign('order_id', $_GET['order_id']);
    $smarty->assign('goods_id', $_GET['goods_id']);
    $smarty->assign('status_order ', $status_order);
    $smarty->assign('bearer_id', $_GET['bearer_id']);
    $smarty->assign('filter', $order_list['filter']);
    $smarty->assign('record_count', $order_list['record_count']);
    $smarty->assign('page_count', $order_list['page_count']);
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('bearer_info.htm');
}
/*------------------------------------------------------ */
//-- 排序、分页、查询
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'info') {
    ...
    $order['pay_time'] = $order['pay_time'] > 0 ?
        local_date($_CFG['time_format'], $order['pay_time']) : $_LANG['ps'][PS_ED];
...
//    $order['status'] = $_LANG['os'][$order['order_status']] . ',' . $_LANG['ps'][$order['pay_status']] . ',' . $_LANG['ss'][$order['shipping_status']];
    $order['status'] = $_LANG['ps'][$order['pay_status']] . ',' . $_LANG['os'][$order['order_status']];
...
//       print_r($order);die;
    ...
    /**获取订单中的酒店信息 start**/
    $smarty->assign('hotel_list', order_hotel($order[order_id]));
    $smarty->assign('hotel_sum', order_hotel_sum($order[order_id]));
    /**获取订单中的酒店信息 end**/
    /**获取订单中的机票信息 start**/

    $smarty->assign('plane_list', order_plane($order[order_id]));
    $smarty->assign('plane_sum', order_plane_sum($order[order_id]));
    /**获取订单中的机票信息 end**/
    /**获取订单中的球票信息   start**/
//    echo "<pre>";
//    print_r(order_combo_menpiao($order[order_id]));die;
    $smarty->assign('ticket_list', order_menpiao($order[order_id]));
    $smarty->assign('combo_list', order_combo_menpiao($order[order_id]));
    $smarty->assign('ticket_sum', order_menpiao_sum($order[order_id]));
    /**获取订单中的球票信息   end**/
    $smarty->assign('goods_attr', $attr);
    $smarty->assign('goods_list', $goods_list);
    $smarty->assign('goods_sum', order_goods_sum($order[order_id]));
    /* 取得能执行的操作列表 */
...
//-- 编辑订单中票务价格信息
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_ticket_price') {
    /* 检查权限 */
    ...
    $goods_price = empty($_POST['val']) ? 0 : floatval($_POST['val']);
    if ($order_id == 0) {
        make_json_error('ERROR');
        exit;
    }else{
        if(!empty($_SESSION) && $_SESSION['action_list'] != 'all'){
            make_json_error('NO PERMISSION');
            exit;
        }
    }

    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_ticket') . " SET goods_price='$goods_price' WHERE order_id = '$order_id' AND goods_id = '$goods_id'";
    if ($GLOBALS['db']->query($sql)) {
        if (empty($goods_price)) {
            make_json_result('N/A');
        } else {
            //门票小计
            $sql = "SELECT SUM(goods_number*goods_price) as order_goods_price FROM sk_order_ticket 
                WHERE order_id = '$order_id' AND goods_id = '$goods_id'";
            $order_subtotal = $GLOBALS['db']->getOne($sql);
            //门票合计
            $ticket_sum = order_menpiao_sum($order_id);
            //总计
            $goods_sum = order_goods_sum($order_id);
            $total_sum = $ticket_sum+$goods_sum;
            $GLOBALS['db']->autoExecute($ecs->table('order_info'), array("goods_amount"=>$total_sum,"order_amount"=>$total_sum), 'UPDATE', "order_id = $order_id");
            $order = order_info($order_id);
            
            $returnArray = array("order_goods_price"=>$goods_price, "order_subtotal"=>$order_subtotal, "order_total"=>$ticket_sum, "order"=>$order);
            make_json_result($returnArray);
        }
    } else {
...
elseif ($_REQUEST['act'] == 'edit_goods_price') {
    ...
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_goods') . " SET goods_price='$goods_price' WHERE order_id = '$order_id' AND goods_id = '$goods_id'";
    if ($GLOBALS['db']->query($sql)) {
        if (empty($goods_price)) {
            make_json_result('N/A');
        } else {
            //周边商品小计
            $sql = "SELECT SUM(goods_number*goods_price) as order_goods_price FROM sk_order_goods
                WHERE order_id = '$order_id' AND goods_id = '$goods_id'";
            $order_subtotal = $GLOBALS['db']->getOne($sql);
            //周边商品合计
            $goods_sum = order_goods_sum($order_id);
            ...
            $ticket_sum = order_menpiao_sum($order_id);
            $total_sum = $ticket_sum+$goods_sum;
            $GLOBALS['db']->autoExecute($ecs->table('order_info'), array("goods_amount"=>$total_sum,"order_amount"=>"1000"), 'UPDATE', "order_id = $order_id");
            $order = order_info($order_id);
            
            $returnArray = array("order_goods_price"=>$goods_price, "order_subtotal"=>$order_subtotal, "order_total"=>$goods_sum, "order"=>$order);
            make_json_result($returnArray);
        }
    } else {
        make_json_error($GLOBALS['db']->errorMsg());
...
//-- 编辑订单中持票人信息票号
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_ticket_code') {
    /* 检查权限 */
    check_authz_json('order_edit');

    $bearer_id = empty($_REQUEST['bearer_id']) ? 0 : intval($_REQUEST['bearer_id']);
    $order_id = empty($_REQUEST['order_id']) ? 0 : intval($_REQUEST['order_id']);
    $goods_id = empty($_REQUEST['goods_id']) ? 0 : intval($_REQUEST['goods_id']);
    if($bearer_id == 0) {
        make_json_error('系统错误，请稍后再试');
        exit;
    }else{
        //查询这张球票的可用票号
        $sql = "SELECT ticket_code FROM ".$GLOBALS['ecs']->table('batch_goods')." WHERE goods_id=$goods_id AND type=0 AND valid=0 ORDER BY id ASC LIMIT 0,1";
        ...
        if($res){
            ...
            $json = new JSON();
            $data = $json->encode('ticket');
            print_r($data);
            exit;
        }
    }
}
elseif ($_REQUEST['act'] == 'baocun_ticket_code') {
    $bearer_id = empty($_REQUEST['bearer_id']) ? 0 : intval($_REQUEST['bearer_id']);
    $order_id = empty($_REQUEST['order_id']) ? 0 : intval($_REQUEST['order_id']);
    $goods_id = empty($_REQUEST['goods_id']) ? 0 : intval($_REQUEST['goods_id']);
    $ticket_code = empty($_REQUEST['ticket_code']) ? "" : json_str_iconv(trim($_REQUEST['ticket_code']));
    if($ticket_code == "" || $bearer_id == 0) {
        make_json_error('系统错误，请稍后再试');
        exit;
    }else{
        //查找票号是否存在
        $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('batch_goods')." WHERE goods_id=$goods_id AND ticket_code='".$ticket_code."' AND type=1 AND valid=0";
        $res = $GLOBALS['db']->getRow($sql);
        if($res){
            $json = new JSON();
            $data = $json->encode('exist');
            print_r($data);
            exit;
        }else{
            //查找票号是否存在
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('batch_goods')." WHERE goods_id=$goods_id AND ticket_code='".$ticket_code."' AND type=0";
            $rs = $GLOBALS['db']->getRow($sql);
        }
        //查找对应的内容
        $sql = " SELECT * FROM ".$GLOBALS['ecs']->table('order_ticket')." WHERE order_id=$order_id AND bearer_id=$bearer_id AND goods_id=$goods_id";
        $ticket = $GLOBALS['db']->getRow($sql);
        if($ticket){
            $sql = "UPDATE ".$GLOBALS['ecs']->table('order_ticket')." SET ticket_code='".$ticket_code."' WHERE order_id=$order_id AND bearer_id=$bearer_id AND goods_id=$goods_id";
            $resu = $GLOBALS['db']->query($sql);
            if($rs){
                //将字段更新
                $sql1 = "UPDATE ".$GLOBALS['ecs']->table('batch_goods')." SET type=1 WHERE ticket_code='".$ticket_code."' AND goods_id=$goods_id";
                ...
                $GLOBALS['db']->autoExecute($ecs->table('batch_goods'), $data, 'INSERT');
            }
            if($resu){
                $json = new JSON();
                $data = $json->encode('ticket');
            ...
            $sql = "UPDATE ".$GLOBALS['ecs']->table('order_combo')." SET ticket_code='".$ticket_code."' WHERE order_id=$order_id AND bearer_id=$bearer_id AND goods_id=$goods_id";
            $resu = $GLOBALS['db']->query($sql);
            if($rs){
                //将字段更新
                $sql1 = "UPDATE ".$GLOBALS['ecs']->table('batch_goods')." SET type=1 WHERE ticket_code='".$ticket_code."' AND goods_id=$goods_id";
...
//点击下载全部持票人信息
elseif($_REQUEST['act'] == "bearerAllCount"){
    $game_id = 38;
    $sql = "SELECT oi.order_sn,bi.* FROM ".$ecs->table('order_info')."AS oi,".$ecs->table('order_ticket')."AS ot,".$ecs->table('bearer_info')."AS bi,".$ecs->table('goods')."AS good,".$ecs->table('game')."AS g WHERE oi.order_id=ot.order_id AND oi.order_status=1 AND oi.pay_status=2 AND ot.goods_id=good.goods_id AND good.goods_id=571 AND good.game_id=g.id AND g.id='".$game_id."' AND ot.bearer_id=bi.id";
    ...
    $objPHPExcel->getActiveSheet()->setTitle('票务销售情况统计');
    $objPHPExcel->getActiveSheet()->setCellValue('A1', '订单号');
    $objPHPExcel->getActiveSheet()->setCellValue('B1', '客户姓名');
    $objPHPExcel->getActiveSheet()->setCellValue('C1', '护照号码');
    $objPHPExcel->getActiveSheet()->setCellValue('D1', '出生日期');
    $objPHPExcel->getActiveSheet()->setCellValue('E1', '护照签发日期');
    $objPHPExcel->getActiveSheet()->setCellValue('F1', '护照有效期');
    $objPHPExcel->getActiveSheet()->setCellValue('G1', '国籍');
    $objPHPExcel->getActiveSheet()->setCellValue('H1', '城市');
    $objPHPExcel->getActiveSheet()->setCellValue('I1', '街道门牌号');
    $objPHPExcel->getActiveSheet()->setCellValue('J1', '邮编');
    $objPHPExcel->getActiveSheet()->setCellValue('K1', '手机号码');
    $objPHPExcel->getActiveSheet()->setCellValue('L1', '座机号码');
    $objPHPExcel->getActiveSheet()->setCellValue('M1', '电子邮件');

    foreach($allData as $key=>$data){
        $objPHPExcel->getActiveSheet()->setCellValue('A'.($key+2), ' '.$data['order_sn']);
        $objPHPExcel->getActiveSheet()->setCellValue('B'.($key+2), $data['cn_customer_name']);
        $objPHPExcel->getActiveSheet()->setCellValue('C'.($key+2), $data['passport_number']);
        $objPHPExcel->getActiveSheet()->setCellValue('D'.($key+2), $data['date_birth']);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.($key+2), $data['issue_date']);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.($key+2), $data['expire_date']);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.($key+2), '   ');
        $objPHPExcel->getActiveSheet()->setCellValue('H'.($key+2), $data['cn_city']);
        $objPHPExcel->getActiveSheet()->setCellValue('I'.($key+2), $data['cn_name_street']);
        $objPHPExcel->getActiveSheet()->setCellValue('J'.($key+2), $data['post_code']);
        $objPHPExcel->getActiveSheet()->setCellValue('K'.($key+2), $data['mobile']);
        $objPHPExcel->getActiveSheet()->setCellValue('L'.($key+2), $data['telephone']);
        $objPHPExcel->getActiveSheet()->setCellValue('M'.($key+2), $data['mail']);
    }
...
//点击下载未中签持票人信息
elseif($_REQUEST['act'] == "bearerIsWinCount"){
    $game_id = 38;
    $sql = "SELECT oi.order_sn,bi.* FROM ".$ecs->table('order_info')."AS oi,".$ecs->table('users')."AS u,".$ecs->table('order_ticket')."AS ot,".$ecs->table('bearer_info')."AS bi,".$ecs->table('goods')."AS good,".$ecs->table('game')."AS g WHERE oi.order_id=ot.order_id AND oi.order_status=1 AND oi.pay_status=2 AND ot.goods_id=good.goods_id AND ot.goods_id=571 AND good.game_id=g.id AND oi.user_id=u.user_id AND u.type=0 AND g.id='".$game_id."' AND ot.bearer_id=bi.id AND oi.is_win=0";
    $allData=$db->getAll($sql);
    __load("phpexcel/PHPExcel", "util");
    __load("phpexcel/PHPExcel/Writer/Excel2007", "util");
    $objPHPExcel = new PHPExcel();
...
//点击下载已中签持票人信息
elseif($_REQUEST['act'] == "bearerWinCount"){
    $game_id = 38;
    $sql = "SELECT oi.order_sn,bi.* FROM ".$ecs->table('order_info')."AS oi,".$ecs->table('order_ticket')."AS ot,".$ecs->table('bearer_info')."AS bi,".$ecs->table('goods')."AS good,".$ecs->table('game')."AS g WHERE oi.order_id=ot.order_id AND oi.order_status=1 AND oi.pay_status=2 AND ot.goods_id=good.goods_id AND good.goods_id=571 AND good.game_id=g.id AND g.id='".$game_id."' AND ot.bearer_id=bi.id AND oi.is_win=1";
    $allData=$db->getAll($sql);
    __load("phpexcel/PHPExcel", "util");
    __load("phpexcel/PHPExcel/Writer/Excel2007", "util");
    $objPHPExcel = new PHPExcel();
...
elseif($_REQUEST['act'] == "order_all_info"){
    $game_id = 38;
//    $time = strtotime("2016-08-01 00:00:00");
    $sql = "SELECT distinct oi.* FROM ".$ecs->table('order_info')."AS oi,".$ecs->table('order_ticket')."AS ot,".$ecs->table('goods')."AS g,".$ecs->table('game')."AS ga WHERE oi.order_id=ot.order_id AND ot.goods_id=g.goods_id AND g.goods_id='571' AND g.game_id=ga.id AND ga.id='".$game_id."'";
//    $sql = "select distinct oi.* from sk_order_info oi,sk_order_ticket ot where oi.order_id=ot.order_id and ot.goods_id=571";
//    $sql = "SELECT oi.* FROM sk_order_info AS oi,sk_order_ticket AS ot,sk_game AS g,sk_goods AS good WHERE oi.order_id=ot.order_id AND oi.add_time>=$time AND ot.goods_id=good.goods_id AND good.game_id=g.id AND g.id='".$game_id."' LIMIT 0,1000";
//    $sql = "SELECT * FROM sk_order_info WHERE add_time>=$time";
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D1', '订单状态');
    $objPHPExcel->getActiveSheet()->setCellValue('E1', '取票状态');
    $objPHPExcel->getActiveSheet()->setCellValue('F1', '支付状态');
    $objPHPExcel->getActiveSheet()->setCellValue('G1', '联系人名称');
    $objPHPExcel->getActiveSheet()->setCellValue('H1', '联系人电话');
    $objPHPExcel->getActiveSheet()->setCellValue('I1', '联系人邮箱');
    $objPHPExcel->getActiveSheet()->setCellValue('J1', '联系人证件类型');
    $objPHPExcel->getActiveSheet()->setCellValue('K1', '联系人证件号码');
    $objPHPExcel->getActiveSheet()->setCellValue('L1', '支付方式');
    $objPHPExcel->getActiveSheet()->setCellValue('M1', '商品金额');
    $objPHPExcel->getActiveSheet()->setCellValue('N1', '订单总金额');
    $objPHPExcel->getActiveSheet()->setCellValue('O1', '发票类型');
    $objPHPExcel->getActiveSheet()->setCellValue('P1', '发票抬头');
    $objPHPExcel->getActiveSheet()->setCellValue('Q1', '发票内容');


    foreach($allData as $key=>$data){
        $objPHPExcel->getActiveSheet()->setCellValue('A'.($key+2),' '.$data['order_id']);
        $objPHPExcel->getActiveSheet()->setCellValue('B'.($key+2), $data['order_sn']);
        $objPHPExcel->getActiveSheet()->setCellValue('C'.($key+2), $data['user_id']);
        $objPHPExcel->getActiveSheet()->setCellValue('D'.($key+2), $data['order_status']);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.($key+2), $data['shipping_status']);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.($key+2), $data['pay_status']);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.($key+2), $data['consignee']);
        $objPHPExcel->getActiveSheet()->setCellValue('H'.($key+2), $data['mobile']);
        $objPHPExcel->getActiveSheet()->setCellValue('I'.($key+2), $data['email']);
        $objPHPExcel->getActiveSheet()->setCellValue('J'.($key+2), $data['card_type']);
        $objPHPExcel->getActiveSheet()->setCellValue('K'.($key+2), $data['card_num']);
        $objPHPExcel->getActiveSheet()->setCellValue('L'.($key+2), $data['pay_name']);
        $objPHPExcel->getActiveSheet()->setCellValue('M'.($key+2), $data['goods_amount']);
        $objPHPExcel->getActiveSheet()->setCellValue('N'.($key+2), $data['order_amount']);
//        $objPHPExcel->getActiveSheet()->setCellValue('O'.($key+2), $data['inv_type']);
        if($data['inv_type']== 发票类型) {
            $objPHPExcel->getActiveSheet()->setCellValue('O' . ($key + 2), " ");
        }else{
            $objPHPExcel->getActiveSheet()->setCellValue('O' . ($key + 2), $data['inv_type']);
        }
        $objPHPExcel->getActiveSheet()->setCellValue('P'.($key+2), $data['inv_payee']);
        $objPHPExcel->getActiveSheet()->setCellValue('Q' . ($key + 2), $data['inv_content']);
    }
    header("Pragma: public");
    header("Expires: 0");
    header("Cache-Control:must-revalidate, post-check=0, pre-check=0");
...
//导入已中签的持票人信息修改订单状态
elseif($_REQUEST['act'] == 'rb_qxx_excel_read') {
    if (! empty ( $_FILES ['file_stu'] ['name'] )) {
        include_once(ROOT_PATH . 'framework/util/phpexcel/PHPExcel.php');
        require_once(ROOT_PATH . 'framework/service/impl/AgentService.php');
        $new_obj = new AgentService();
...
//            show_message("上传文件失败，请重新上传",$url);
            sys_msg($_LANG['show_excel_fail'], 0 );   // 记录管理员操作
        }
        ...
        foreach ( $data_array as $k => $v ) {
            ...
            $game_id = 38;
            $order_id = $GLOBALS['db']->getOne("SELECT oi.order_id FROM sk_order_info AS oi,sk_order_ticket AS ot,sk_goods AS g,sk_bearer_info AS bi WHERE bi.passport_number='".$passport_number."' AND ot.goods_id=g.goods_id AND g.goods_id=571 AND g.game_id='".$game_id."' AND bi.id=ot.bearer_id AND ot.order_id=oi.order_id AND oi.order_status=1 AND oi.pay_status=2");
            $sql = "UPDATE sk_order_info SET is_win=1 WHERE order_id='".$order_id."'";
            $GLOBALS['db']->query($sql);
            //获取联系人信息
            $customer = get_bearer_info($order_id);
            Logger::DEBUG("user mobile and email:".$customer['mobile'].$customer['email']);
            $sql = "SELECT is_win FROM ".$GLOBALS['ecs']->table('order_info')."WHERE order_id='".$order_id."'";
            $rs = $GLOBALS['db']->getOne($sql);
            if($rs['is_win'] == 1){
                require_once(ROOT_PATH . 'includes/lib_sms.php');
                if(substr($customer['mobile'],0,2) == '86'){
                    Logger::DEBUG("user mobile :".$customer['mobile']);
                    sendsms($customer['mobile'], '恭喜您已成功购得世预赛中国足协中国之队客场球票。取票详情请关注“盛开官方赛事票务款待”公众号、购票官网。');
                }else{
                    sendsms($customer['mobile'], 'Congratulations! You are the successful applicant to get the ticket of the Asian Qualifiers Uzbekistan vs China. Please follow the Wechat and website of "Shankai Hospitality", and the tickets details will be announced separately.');
                }
//                if(substr($cunstomer['mobile'],0,2) == '82'){
//                    sendsms($customer['mobile'], 'Congratulations! You are the successful applicant to get the ticket of the Asian Qualifiers Korea vs China. Please follow the Wechat and website of "Shankai Hospitality", and the tickets details will be announced separately.');
//                }else{
//                    sendsms($customer['mobile'], "恭喜您已成功中签获得世预赛中国足协中国之队客场对阵韩国国家队门票。请您关注“盛开官方赛事票务款待”公众号、购票官网，取票详情另行公布。");
//                }

                $new_obj->send_mail('Success', array("to" => array($customer['email'])));
            }
        }
        }else{
            //            show_message("上传的文件不是.xls类型，请重新上传",$url);
            sys_msg($_LANG['show_excel_success'], 0 );   // 记录管理员操作
        }

        unlink($savePath . $file_name);
        sys_msg($_LANG['leadexcel_success'], 0 );   // 记录管理员操作
        clear_cache_files();    // 清除缓存
    }else{
        sys_msg($_LANG['leadexcel_fail'], 0 );   // 记录管理员操作
    }
...
//        /* 商家发货时给客户发送短信提醒 */
//        if ($_CFG['ecsdxt_order_shipped'] == '1' && $order['mobile'] != '') {
//            require_once(ROOT_PATH . 'includes/lib_sms.php');
//
//            $smarty->assign('shop_name', $_CFG['shop_name']);
...
//
//            $content = $smarty->fetch('str:' . $_CFG['ecsdxt_order_shipped_value']);
//
...
//                update_order_amount($order_id);
                ...
                Logger::DEBUG("gou piao ping zheng sheng cheng :".$order['order_sn']);
//                //发送短信
                //查询订单下的所有持票人
                ...
                if(substr($order['mobile'],0,2) == '86'){
                    sendsms($order['mobile'], "您的订单（{$order['order_sn']}）持票人已审核成功，您可以在个人中心查看和下载购票凭证。");
                    Logger::DEBUG("mobile send:".$order['mobile'].$order['order_sn']);
                }else{
                    sendsms($order['mobile'], "The holder information of the order {$order['order_sn']} has been successfully approved. Please check and download the ticket voucher in the personal center.");
                    Logger::DEBUG("mobile send:".'----------1'.$order['mobile'].$order['order_sn']);
                }
//                if(substr($order['mobile'],0,2) == '82'){
//                    sendsms($order['mobile'], "The holder's information of the order {$order['order_sn']} has been successfully approved. Please check and download the ticket voucher in the personal center.");
//                    Logger::DEBUG("mobile send:".$order['mobile'].$order['order_sn']);
//                }else{
//                    sendsms($order['mobile'], "您的订单（{$order['order_sn']}）持票人已审核成功，您可以在个人中心查看和下载购票凭证。");
//                    Logger::DEBUG("mobile send:".$order['mobile'].$order['order_sn']);
...
//                __load("BearerService");
//
//                die('1321312111111111');
//                $bearer = new BearerService();

                $bearer_obj->send_mail('shenhe_ok', array("to" => array($order['email']), "sub" => array('%order_sn%' => array($order['order_sn']))));
                Logger::DEBUG("email send:".$order['email']);
...
//                }
                ...
                if ($order['txnAmt'] > 0) {
                    ...
                    if ($payment['pay_code'] = "unionpay_Online") {
                        $pay_online = $pay_obj->ConsumeUndo($order);
                    }
                }
...
elseif ($_REQUEST['act'] == 'operate_post') {
    ...
    /*查询所有的持票人*/
    $order_bearer_info_list = get_order_bearer_info($order);
    /* 取得备注信息 */
    $action_note = $_REQUEST['action_note'];
    ...
    if ('confirm' == $operation) {
         ...
         * （套餐和单票）
         */
        $combo_ticket = get_combo_num($order_id);
        $ticket = get_ticket_num($order_id);
        if($combo_ticket){
            foreach($combo_ticket as $k1=>$v1){
                //查询当前订单中所有属于这个商品的票
                $ticket_list = $GLOBALS['db']->getAll("SELECT * FROM sk_order_combo WHERE goods_id=$v1[goods_id] AND order_id=$order_id");
                //查找对应的商品票号
                $batch_code_list = $GLOBALS['db']->getAll("SELECT * FROM sk_batch_goods WHERE goods_id=$v1[goods_id] AND type=0 AND valid=0 ORDER BY id ASC LIMIT 0,".$v1['combo_goods_num']);
                if($batch_code_list) {
                    foreach ($ticket_list as $k7 => $v7) {
                        if($batch_code_list[$k7]['ticket_code'] == ''){
                            $sql = "UPDATE " . $GLOBALS['ecs']->table('order_combo') . " SET ticket_code='"."'"." WHERE goods_id=$v7[goods_id] AND order_id=$order_id AND rec_id=$v7[rec_id]";
                            $GLOBALS['db']->query($sql);
                        }else {
                            $sql = "UPDATE " . $GLOBALS['ecs']->table('order_combo') . " SET ticket_code='" . $batch_code_list[$k7]['ticket_code'] . "' WHERE goods_id=$v7[goods_id] AND order_id=$order_id AND rec_id=$v7[rec_id]";
                            $GLOBALS['db']->query($sql);
                            foreach ($batch_code_list as $k2 => $v2) {
                                $sql = "UPDATE sk_batch_goods SET type=1 WHERE goods_id=$v2[goods_id] AND ticket_code='" . $v2['ticket_code']."'";
                                $GLOBALS['db']->query($sql);
                            }
                        }
                    }
        ...
        if($ticket){
            foreach($ticket as $k3=>$v3){
                //查询当前订单中所有属于这个商品的票
                $ticket_list1 = $GLOBALS['db']->getAll("SELECT * FROM sk_order_ticket WHERE goods_id=$v3[goods_id] AND order_id=$order_id");
                //查找对应的商品票号
                $batch_code_list1 = $GLOBALS['db']->getAll("SELECT * FROM sk_batch_goods WHERE goods_id=$v3[goods_id] AND type=0 AND valid=0 ORDER BY id ASC LIMIT 0,".$v3['ticket_goods_num']);
                if($batch_code_list1) {
                    foreach ($ticket_list1 as $k5 => $v5) {
                        if($batch_code_list1[$k5]['ticket_code'] == ''){
                            $sql = "UPDATE " . $GLOBALS['ecs']->table('order_ticket') . " SET ticket_code='"."'"." WHERE goods_id=$v5[goods_id] AND order_id=$order_id AND rec_id=$v5[rec_id]";
                            $GLOBALS['db']->query($sql);
                        }else {
                            $sql = "UPDATE " . $GLOBALS['ecs']->table('order_ticket') . " SET ticket_code='" . $batch_code_list1[$k5]['ticket_code'] . "' WHERE goods_id=$v5[goods_id] AND order_id=$order_id AND rec_id=$v5[rec_id]";
                            $GLOBALS['db']->query($sql);
                            foreach ($batch_code_list1 as $k4 => $v4) {
                                $sql = "UPDATE sk_batch_goods SET type=1 WHERE goods_id=$v4[goods_id] AND ticket_code='".$v4['ticket_code']."'";
                                $GLOBALS['db']->query($sql);
                            }
                        }
                    }
        ...
        //生成持票人pdf
        $sql = "SELECT a.*,b.ticket_code FROM " . $GLOBALS['ecs']->table('bearer_info') . " AS a" .
                " LEFT JOIN " . $GLOBALS['ecs']->table('order_ticket') ." AS b ON b.order_id = a.order_id AND b.bearer_id = a.id" .
                " WHERE b.order_id =" .$order_id;
        $bearer_array = $GLOBALS['db']->getAll($sql);
        foreach($bearer_array as $bearer_info){
            $smarty->assign("info", $bearer_info);
            sava_pdf($smarty->fetch("pdf_tpl/pdf.tpl", null), ROOT_PATH."data/pdf_data/user_info/user_bearer_{$bearer_info['id']}.pdf", true, false);
        }
        //生成购票凭证pdf
        create_vouch_pdf($order_id);
        Logger::DEBUG($order['order_sn']."sheng cheng gou piao ping zheng :");
        //update_order_amount($order_id);
        require_once(ROOT_PATH . 'includes/lib_sms.php');
        //----hechengbin --- start ---
        if(substr($order['mobile'],0,2) == '86'){
            sendsms($order['mobile'], "您的订单（{$order['order_sn']}）持票人已审核成功，您可以在个人中心查看和下载购票凭证。");
            Logger::DEBUG("mobile send:".$order['mobile']);
        }else{
            sendsms($order['mobile'], "The holder information of the order {$order['order_sn']} has been successfully approved. Please check and download the ticket voucher in the personal center.");
            Logger::DEBUG("act = operate_post  : mobile send:".'-------2'.$order['mobile']);
        }
        foreach($order_bearer_info_list as $kk=>$vv){
            if(substr($order_bearer_info_list[$kk]['mobile'],0,2) == '86'){
                sendsms($order_bearer_info_list[$kk]['mobile'], "您的个人信息已审核成功，请您联系购票人索取购票凭证。");
                Logger::DEBUG("mobile send:".$order_bearer_info_list[$kk]['mobile']);
                $bearer_obj->send_mail('shenhe_success', array("to" => array($order_bearer_info_list[$kk]['mail'])));
                Logger::DEBUG("email send success:".$order_bearer_info_list[$kk]['mail'] .'--' .'success');
            }else{
                sendsms($order_bearer_info_list[$kk]['mobile'],"Your personal information has been approved. Please contact tickets holder and download voucher for your reference.");
                Logger::DEBUG("mobile send:".$order_bearer_info_list[$kk]['mobile']);
                $bearer_obj->send_mail('verify_success', array("to" => array($order_bearer_info_list[$kk]['mail'])));
                Logger::DEBUG("email send success:".$order_bearer_info_list[$kk]['mail'] .'--' .'success');
            }
//            $bearer_obj->send_mail('shenhe_ok', array("to" => array($order_bearer_info_list[$kk]['email']), "sub" => array('%order_sn%' => array($order['order_sn']))));
//            Logger::DEBUG("email send success:".$order['email']);
        }
//        if(substr($order['mobile'],0,2) == '82'){
//            $ret = sendsms($order['mobile'], "The holder's information of the order {$order['order_sn']} has been successfully approved. Please check and download the ticket voucher in the personal center.");
//            Logger::DEBUG("act = operate_post  : mobile send:".$order['mobile']);
//        }else{
//            $ret = sendsms($order['mobile'], "您的订单（{$order['order_sn']}）持票人已审核成功，您可以在个人中心查看和下载购票凭证。");
//            Logger::DEBUG("mobile send:".$order['mobile']);
//        }
        //----hechengbin --- end ---
//        $ret = sendsms($order['mobile'], "您的订单（{$order['order_sn']}）持票人已审核成功，您可以在个人中心查看和下载购票凭证。");
        ...
        Logger::DEBUG("email send success:".$order['email']);
        /* 发送邮件 */
        //  $bearer_obj->send_mail('pay_ok2', array("to" => array($order['email']), "sub" => array('%order_sn%' => array($order['order_sn']))));
    } /* 付款 */
...
//        }
        ...
        //----hechengbin --- start ---
        if(substr($order['mobile'],0,2) == '86'){
            sendsms($order['mobile'], "我们已收到您订单为{$order['order_sn']}的款项，订单金额为￥{$order['order_amount']}元。请您在72小时之内前往个人中心填写持票人信息。");
            Logger::DEBUG("mobile send:".'----------4'.$order['mobile']);
        }else{
            sendsms($order['mobile'], "Payment received for the order {$order['order_sn']} . Please fill out the holder information in 72 hours in the personal center.");
            Logger::DEBUG("mobile send:".'----------3'.$order['mobile']);
        }
//        if(substr($order['mobile'],0,2) == '82'){
//            $ret = sendsms($order['mobile'], "Payment received for the order {$order['order_sn']}. Please fill out the holder's information in 72 hours in the personal center.");
//        }else{
//            $ret = sendsms($order['mobile'], "我们已收到您订单为{$order['order_sn']}的款项，订单金额为￥{$order['order_amount']}元。请您在72小时之内前往个人中心填写持票人信息。");
//        }
        //----hechengbin --- end ---

       // $ret = sendsms($order['mobile'], "我们已收到您订单为{$order['order_sn']}的款项，请您在72小时之内前往个人中心填写持票人信息。");
        /* 记录log */
        order_action($order['order_sn'], OS_UNCONFIRMED, $order['shipping_status'], PS_PAYED, $action_note);
    } /* 设为未付款 */
    ...
    elseif ('cancel' == $operation) {
         ...
         */
        //查询订单所有票号
        $ticket_code_list = get_ticket_code($order_id);
        $combo_code_list = get_combo_code($order_id);
        $code = array_merge($ticket_code_list,$combo_code_list);
        foreach($code as $k=>$v){
            //更新票号表中票号为可用状态
            $sql = "UPDATE ".$GLOBALS['ecs']->table('batch_goods')." SET type=0 WHERE ticket_code='".$v['ticket_code']."' AND valid=0 AND goods_id=$v[goods_id]";
            $GLOBALS['db']->query($sql);
        }
        /* 记录log */
        order_action($order['order_sn'], OS_CANCELED, $order['shipping_status'], PS_UNPAYED, $action_note);
...
elseif ($_REQUEST['act'] == 'get_goods_info') {
    ...
    $smarty->assign('ticket_list', order_menpiao($order_id));
    $smarty->assign('goods_attr', $attr);
    $smarty->assign('goods_list', $goods_list);
 ...
 * @return array
 */
function order_list($order_type = 1)
{
    if ($order_type == 3) {//纪念品订单
        return order_list_goods();
    } elseif ($order_type == 4) {//机票订单
        return order_list_plane();
    } elseif ($order_type == 5) {//酒店订单
        return order_list_hotel();
    } else {
        return order_list_default();
    }
}
 ...
 * @return array
 */
function order_list_plane()
{
    $result = get_filter();
    if ($result === false) {
        /* 过滤信息 */
        $filter['order_sn'] = empty($_REQUEST['order_sn']) ? '' : trim($_REQUEST['order_sn']);
        if (strlen($filter['order_sn']) > 13) {
            $filter['order_sn'] = substr($filter['order_sn'], 0, 13);
        }
        ...
        $filter['mobile'] = empty($_REQUEST['mobile']) ? 0 : ($_REQUEST['mobile']);

        $filter['start_amount'] = empty($_REQUEST['start_amount']) ? '' : ($_REQUEST['start_amount']);
        $filter['end_amount'] = empty($_REQUEST['end_amount']) ? '' : ($_REQUEST['end_amount']);

        $filter['ticket'] = empty($_REQUEST['ticket']) ? '' : intval($_REQUEST['ticket']);
...
//        $filter['rank'] = empty($_REQUEST['rank']) ?  0 : trim($_REQUEST['rank']);
        $rank = empty($_REQUEST['rank']) ? '' : trim($_REQUEST['rank']);
        $rank_1 = empty($_REQUEST['rank_1']) ? '' : trim($_REQUEST['rank_1']);
        ...
        $filter['code_status'] = isset($_REQUEST['code_status']) ? intval($_REQUEST['code_status']) : -1;//票号录入状态
        $filter['source'] = empty($_REQUEST['source']) ? 0 : intval($_REQUEST['source']);
        $filter['goods_number'] = empty($_REQUEST['goods_number']) ? 0 : intval($_REQUEST['goods_number']);
        $filter['is_goods'] = empty($_REQUEST['is_goods']) ? 0 : intval($_REQUEST['is_goods']);
        ...
        $where = 'WHERE 1 = 1 and o.source=1 ';
        if(!empty($_REQUEST['third_platform']) && $_REQUEST['third_platform'] > 0){
            $where .= " and o.third_platform='".$_REQUEST['third_platform']."'";
            $filter['third_platform'] = $_REQUEST['third_platform'];
        }
        ...
        if ($filter['source'] == 2) {
            $where .= " AND o.is_pc_mobile = 2 ";
            $where_list .= " AND a.is_pc_mobile = 2";
        } elseif ($filter['source'] == 1) {
            $where .= " AND o.is_pc_mobile = 1 ";
            $where_list .= " AND a.is_pc_mobile = 1 ";
        }
        if ($filter['composite_status'] == 1) {
            $where .= " AND o.pay_status=0 AND o.order_status !=2";
            $where_list .= " AND a.pay_status=0 AND a.order_status !=2";
        } else if ($filter['composite_status'] == 2) {
            $where .= " AND o.order_status=0 AND o.pay_status = 2";
            $where_list .= " AND a.order_status=0 AND a.pay_status = 2";
        } else if ($filter['composite_status'] == 3) {
            $where .= " AND o.order_status=1 AND o.pay_status = 2";
            $where_list .= " AND a.order_status=1 AND a.pay_status = 2";
        } elseif ($filter['composite_status'] == 4) {
            $where .= " AND o.order_status=2";
            $where_list .= " AND a.order_status=2";
        }
        if ($filter['email']) {
        ...
        if ($filter['goods_id']) {
            $where .= " AND op.air_id = '$filter[goods_id]'";
        }
        ...
        /* 记录总数 */
        $sql = 'SELECT COUNT(DISTINCT op.order_id) FROM sk_order_plane op RIGHT join sk_order_info o on op.order_id = o.order_id ' . $where;
        ...
        $sql = "
                ...
                IFNULL(b.user_name,'匿名用户') AS buyer,
                o.spm
                FROM sk_order_plane op
                LEFT JOIN
                sk_order_info o
        ...
        foreach (array('order_sn', 'consignee', 'goods_amount ', 'email', 'address', 'zipcode', 'tel', 'user_name') AS $val) {
...
//        $filter['rank'] = empty($_REQUEST['rank']) ?  0 : trim($_REQUEST['rank']);
        ...
        if ($filter['goods_id']) {
            $where .= " AND oH.hotel_id = '$filter[goods_id]'";
        }
        ...
        /* 记录总数 */
        $sql = 'SELECT COUNT(DISTINCT oh.order_id) FROM sk_order_hotel oh RIGHT join sk_order_info o on oh.order_id = o.order_id ' . $where;
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
...
//        $filter['rank'] = empty($_REQUEST['rank']) ?  0 : trim($_REQUEST['rank']);
        ...
        $where = 'WHERE g.is_ticket = 0 AND g.is_goods = 1 and o.source=1';
        if(!empty($_REQUEST['third_platform']) && $_REQUEST['third_platform'] > 0){
            $where .= " and o.third_platform='".$_REQUEST['third_platform']."'";
            $filter['third_platform'] = $_REQUEST['third_platform'];
        }
        ...
        if ($filter['game_id'] != 0) {
            $where_list .= " AND g.game_id = '$filter[game_id]'";
        }
        if ($filter['number_id'] != 0) {
            $where_list .= " AND g.number_id = '$filter[number_id]'";
        }
        if ($filter['pitch_id'] != 0) {
            $where_list .= " AND g.pitch_id = '$filter[pitch_id]'";
        }
        if ($filter['order_status'] != -1) {
            $where .= " AND o.order_status  = '$filter[order_status]'";
        }
        ...
        if ($filter['goods_id']) {
            $where .= " AND og.goods_id = '$filter[goods_id]'";
        }
        if ($filter['goods_sn']) {
            $where .= " AND og.goods_sn = '$filter[goods_sn]'";
        }

        $sql2 = 'SELECT * FROM sk_order_goods og left join sk_goods g on og.goods_id = g.goods_id right join sk_order_info o on o.order_id = og.order_id where g.is_ticket = 0 and g.is_goods = 1';
        $group_by = ' group by og.order_id';
        ...
        /* 记录总数 */
        $sql = 'SELECT COUNT(DISTINCT og.order_id) FROM sk_order_goods og left join sk_goods g on og.goods_id = g.goods_id RIGHT JOIN sk_order_info o ON og.order_id = o.order_id ' . $where;
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
...
//        $filter['rank'] = empty($_REQUEST['rank']) ?  0 : trim($_REQUEST['rank']);
        ...
        if ($filter['goods_sn']) {
            $sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_sn = '" .$filter['goods_sn'] . "'";
        ...
        if ($filter['start_amount'])
        {
            $where .= " AND o.goods_amount >= '$filter[start_amount]'";
        }
        if ($filter['end_amount'])
        {
            $where .= " AND o.goods_amount <= '$filter[end_amount]'";
        }
        if ($filter['email']) {
            $where .= " AND o.email LIKE '%" . mysql_like_quote($filter['email']) . "%'";
        }
        ...
        if($filter['game_id'] != 0){
            $where .= " AND g.game_id = '$filter[game_id]'";
            $where_list .= " AND d.game_id = '$filter[game_id]'";
        }
        if($filter['number_id'] != 0){
            $where .= " AND g.number_id = '$filter[number_id]'";
            $where_list .= " AND d.number_id = '$filter[number_id]'";
        }
        if($filter['pitch_id'] != 0){
            $where .= " AND n.pitch_id = '$filter[pitch_id]'";
            $where_list .= " AND f.pitch_id = '$filter[pitch_id]'";
        }
        if ($filter['country']) {
...
//        if ($filter['goods_id']) {
//            $where .= " AND ot.goods_id = '$filter[goods_id]'";
//        }
//
        if ($filter['goods_id']) {
            if($_REQUEST['is_goods'] == 1){
...
//}
        ...
        {
            $where .= " AND b.cn_customer_name LIKE '%" . mysql_like_quote($filter['cn_customer_name']) . "%'";
        }
        if ($filter['rank']) {
            $where .= " AND g.rank='".$filter['rank']. "'";
            $where_list .= " AND d.rank='".$filter['rank']. "'";
        }
        if($filter['ticket'] == 1){
            $where .= " AND o.order_id = oc.order_id ";
        }
        if($filter['ticket'] == 2){
            $where .= " AND o.order_id = ot.order_id ";
        }
...
//
//        //票号录入状态
//        switch ($filter['code_status']) {
//            case 0:
//                $where .= " AND p.ticket_code = ''";
//                break;
//            case 1:
//                $where .= " AND p.ticket_code != ''";
//                break;
...
//        }
        ...
        if($filter['user_name']){
            $sql ="SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('order_info')." AS o LEFT JOIN ".$GLOBALS['ecs']->table('users')." AS u ON o.user_id=u.user_id " . $where;

        }elseif($filter['goods_id']){
            //判断商品类型：球票or周边商品
            if($_REQUEST['is_goods'] == 1){
                //周边商品
                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                    " LEFT JOIN " . $GLOBALS['ecs']->table('order_goods') . " AS og ON og.order_id=o.order_id " .$where;

            }else{
                //门票
                $sql = "SELECT COUNT(DISTINCT order_sn) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                    " LEFT JOIN " . $GLOBALS['ecs']->table('order_ticket') . " AS ot ON ot.order_id=o.order_id " .$where;

            }
        }elseif($filter['game_id'] == 0 && $filter['number_id'] == 0 && $filter['pitch_id'] == 0 && $filter['ticket'] == 1){
            $sql = "SELECT COUNT(DISTINCT oc.order_id) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o," . $GLOBALS['ecs']->table('order_combo') . " AS oc " .$where;
        }elseif(($filter['game_id'] != 0 || $filter['number_id'] != 0 || $filter['pitch_id'] != 0) && $filter['ticket'] == 1){
            $sql = "SELECT COUNT(DISTINCT oc.order_id) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o," . $GLOBALS['ecs']->table('order_combo') . " AS oc ,".$GLOBALS['ecs']->table('goods')." AS g,".$GLOBALS['ecs']->table('number')." AS n " .$where." AND oc.goods_id=g.goods_id";
        }elseif($filter['game_id'] == 0 && $filter['number_id'] == 0 && $filter['pitch_id'] == 0 && $filter['ticket'] == 2){
            $sql = "SELECT COUNT(DISTINCT ot.order_id) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o," . $GLOBALS['ecs']->table('order_ticket') . " AS ot " .$where;
        }elseif(($filter['game_id'] != 0 || $filter['number_id'] != 0 || $filter['pitch_id'] != 0) && $filter['ticket'] == 2){
            $sql = "SELECT COUNT(DISTINCT o.order_sn) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o LEFT JOIN " .
                $GLOBALS['ecs']->table('order_ticket') . " AS ot ON o.order_id=ot.order_id LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON ot.goods_id=g.goods_id LEFT JOIN ".$GLOBALS['ecs']->table('number')." AS n ON g.number_id=n.id " . $where;
        }elseif(($filter['game_id'] != 0 || $filter['number_id'] != 0 || $filter['pitch_id'] != 0) && $filter['rank']){
            $sql = "SELECT COUNT(DISTINCT o.order_sn) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o LEFT JOIN " .
                $GLOBALS['ecs']->table('order_ticket') . " AS ot ON o.order_id=ot.order_id LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON ot.goods_id=g.goods_id LEFT JOIN ".$GLOBALS['ecs']->table('number')." AS n ON g.number_id=n.id " . $where;
        }elseif($filter['cn_customer_name']) {
            $sql = "SELECT COUNT(DISTINCT o.order_sn) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " ." LEFT JOIN ".
                $GLOBALS['ecs']->table('order_ticket') . " AS ot ON ot.order_id = o.order_id LEFT JOIN ".$GLOBALS['ecs']->table('bearer_info') ."AS b ON ot.bearer_id=b.id " .$where;
        }elseif($filter['goods_number']){
            $sql = "SELECT oi.* FROM ".$GLOBALS['ecs']->table('order_info')." AS oi LEFT JOIN ".$GLOBALS['ecs']->table('order_ticket')." AS ot ON oi.order_id=ot.order_id group by ot.order_id having  count(ot.order_id)=".$filter['goods_number'];
        }elseif($filter['game_id'] != 0  || $filter['number_id'] != 0 || $filter['pitch_id'] != 0){
            $sql = "SELECT COUNT(DISTINCT oi.order_sn) FROM " . $GLOBALS['ecs']->table('order_info') . " AS oi LEFT JOIN " .
                $GLOBALS['ecs']->table('order_ticket') . " AS ot ON oi.order_id=ot.order_id LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON ot.goods_id=g.goods_id LEFT JOIN ".$GLOBALS['ecs']->table('number')." AS n ON g.number_id=n.id " . $where;
        }
        elseif($filter['code_status'] != -1){
            //票号录入情况
        ...
        if($_REQUEST['is_goods'] == -1){
            $ordersql = " AND o.order_status != 2";
        }
//        echo $sql;
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
        $filter['page_count'] = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        if($filter['is_goods'] == 1){
            $joinSql = " LEFT JOIN " . $GLOBALS['ecs']->table('order_goods') . " AS og ON og.order_id=o.order_id " ;
        }elseif($filter['is_goods'] == -1){
            $joinSql = " LEFT JOIN " . $GLOBALS['ecs']->table('order_ticket') . " AS ot ON ot.order_id=o.order_id " ;
        }else{
            $joinSql = '';
        }

        if($filter['game_id'] != -1 && $filter['game_id'] != 0){
            $joinSql = " LEFT JOIN " .$GLOBALS['ecs']->table('order_ticket'). " AS ot ON ot.order_id=o.order_id LEFT JOIN " .$GLOBALS['ecs']->table('goods')." AS g ON ot.goods_id=g.goods_id";
        }
        if($filter['number_id'] != -1 && $filter['number_id'] != 0){
            $joinSql = " LEFT JOIN " .$GLOBALS['ecs']->table('order_ticket'). " AS ot ON ot.order_id=o.order_id LEFT JOIN " .$GLOBALS['ecs']->table('goods')." AS g ON ot.goods_id=g.goods_id";
        }
        if($filter['pitch_id'] != -1 && $filter['pitch_id'] != 0){
            $joinSql = " LEFT JOIN " .$GLOBALS['ecs']->table('order_ticket'). " AS ot ON ot.order_id=o.order_id LEFT JOIN " .$GLOBALS['ecs']->table('goods')." AS g ON ot.goods_id=g.goods_id LEFT JOIN " .$GLOBALS['ecs']->table('number')." AS n ON g.number_id=n.id";
        }
        if($filter['cn_customer_name']){
            $ticketsql = " LEFT JOIN" .$GLOBALS['ecs']->table('order_ticket')." AS ot ON ot.order_id=o.order_id" ." LEFT JOIN ".$GLOBALS['ecs']->table('bearer_info')." AS b ON b.id=ot.bearer_id ";
        }
        if($filter['code_status'] != -1){
            $codeSql = " LEFT JOIN " . $GLOBALS['ecs']->table('order_ticket') . " AS p ON p.order_id=o.order_id " .
                " LEFT JOIN " . $GLOBALS['ecs']->table('bearer_info') . " AS j ON j.id=p.bearer_id ";
        }else{
            $codeSql = '';
        }
        if($filter['goods_number']){
            $joinSql = " LEFT JOIN " .$GLOBALS['ecs']->table('order_ticket'). " AS ot ON o.order_id=ot.order_id group by ot.order_id having  count(ot.order_id)=" .$filter['goods_number'];
        }
        if($filter['ticket'] == 1){
            $joinSql = "LEFT JOIN ".$GLOBALS['ecs']->table('order_combo')." AS oc ON oc.order_id=o.order_id";
        }elseif($filter['ticket'] == 2){
            $joinSql = "LEFT JOIN ".$GLOBALS['ecs']->table('order_ticket')." AS ot ON ot.order_id=o.order_id";
        }

        /* 查询 */
        if($filter['goods_number'] != 0){
            $sql = "SELECT distinct o.order_id, o.is_win, o.goods_amount, o.order_sn,o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code , o.order_status, o.extension_id, " .
                "(" . order_amount_field('o.') . ") AS total_fee, o.spm " .
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " . $joinSql .$ticketsql .$ordersql .
                " ORDER BY $filter[sort_by] $filter[sort_order] " .
                " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
        }elseif($filter['game_id'] == 0 && $filter['number_id'] == 0 && $filter['pitch_id'] == 0 && $filter['ticket'] == 1){
            $sql = "SELECT distinct o.order_sn, o.order_id, o.is_win, o.goods_amount, o.order_sn,o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code , o.order_status, o.extension_id, " .
                "(" . order_amount_field('o.') . ") AS total_fee, " .
                "IFNULL(u.user_name, '" . $GLOBALS['_LANG']['anonymous'] . "') AS buyer, o.spm " .
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " . $joinSql .
                " LEFT JOIN " . $GLOBALS['ecs']->table('users') . " AS u ON u.user_id=o.user_id " . $ticketsql . $where . $ordersql .
                " ORDER BY $filter[sort_by] $filter[sort_order] " .
                " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
        }elseif(($filter['game_id'] != 0 || $filter['number_id'] != 0 || $filter['pitch_id'] != 0 || $filter['source'] !=0 || $filter['rank'] || $filter['composite_status'] !=0)&& $filter['ticket'] == 1){
            $sql = "select
                    DISTINCT
                    a.order_sn,
                    a.order_id,
                    ...
                    a.order_status,
                    a.extension_id,
                    (a.goods_amount+a.tax+a.shipping_fee+a.insure_fee+a.pay_fee+a.pack_fee+a.card_fee) AS total_fee,
                    IFNULL(b.user_name,'匿名用户') AS buyer,
                    a.spm
                from
                    sk_order_info a,
                ...
                where
                  ...
                  ".$where_list."
                    ORDER BY $filter[sort_by] $filter[sort_order] " .
                " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
        }elseif(($filter['game_id'] != 0 || $filter['number_id'] != 0 || $filter['pitch_id'] != 0 || $filter['source'] !=0 ||  $filter['rank'] || $filter['composite_status'] != 0) && $filter['ticket'] == 2){
            $sql = "select
                    DISTINCT
                    a.order_sn,
                    a.order_id,
...
function delivery_list()
{
    ...
    if ($result === false) {
        ...
        /* 查询 */
        $sql = "SELECT delivery_id, delivery_sn, order_sn, order_id, add_time, action_user, consignee, mobile,country,
...
//根据商品id获取到所有的订单
function order_list_goods_info($id){
    $order_list['ticket'] = $GLOBALS['db']->getAll("SELECT oi.*,ot.goods_id,ua.* FROM ".$GLOBALS['ecs']->table('order_ticket')."AS ot,".$GLOBALS['ecs']->table('order_info')."AS oi,".$GLOBALS['ecs']->table('user_address')."AS ua WHERE ot.goods_id='".$id."' AND ot.order_id=oi.order_id AND oi.address_id=ua.address_id");
    $order_list['combo']= $GLOBALS['db']->getAll("SELECT oi.*,oc.goods_id,ua.* FROM ".$GLOBALS['ecs']->table('order_combo')."AS oc,".$GLOBALS['ecs']->table('order_info')."AS oi,".$GLOBALS['ecs']->table('user_address')."AS ua WHERE oc.goods_id='".$id."' AND oc.order_id=oi.order_id AND oi.address_id=ua.address_id");
    $order_list['data'] = array_merge($order_list['ticket'],$order_list['combo']);
    $order_list['goods'] = $GLOBALS['db']->getAll("SELECT order_sn FROM ".$GLOBALS['ecs']->table('order_goods')."AS og,".$GLOBALS['ecs']->table('order_info')."AS oi WHERE og.goods_id='".$id."' AND og.order_id=oi.order_id");
    return $order_list;
}

function bearer_info_order_id($order_id){
    $ticket_bearer_list = $GLOBALS['db']->getAll("SELECT bi.*,ot.goods_id,ot.ticket_code FROM " .$GLOBALS['ecs']->table('bearer_info')."AS bi,".$GLOBALS['ecs']->table('order_ticket')."AS ot WHERE ot.order_id='".$order_id."' AND ot.bearer_id=bi.id");
    $combo_bearer_list = $GLOBALS['db']->getAll("SELECT bi.*,oc.goods_id,oc.ticket_code FROM " .$GLOBALS['ecs']->table('bearer_info')."AS bi,".$GLOBALS['ecs']->table('order_combo')."AS oc WHERE oc.order_id='".$order_id."' AND oc.bearer_id=bi.id");
    return array_merge($ticket_bearer_list,$combo_bearer_list);
}
//获取订单的联系人信息
function get_bearer_info($order_id){
    $sql = "SELECT oi.* FROM ".$GLOBALS['ecs']->table('users')."AS u,".$GLOBALS['ecs']->table('order_info')."AS oi WHERE oi.order_id='".$order_id."' AND oi.user_id=u.user_id";
    return $GLOBALS['db']->getRow($sql);
}
//查看已经取票的订单和持票人
function get_order_status(){
    $order_status['order_status'] = $GLOBALS['db']->getOne("SELECT count(order_sn) FROM ".$GLOBALS['ecs']->table('order_info')." WHERE shipping_status=3");
    $order_status['order_bearer'] = $GLOBALS['db']->getOne("SELECT count(bi.id) FROM ".$GLOBALS['ecs']->table('order_info')."AS oi,".$GLOBALS['ecs']->table('order_ticket')."AS ot,".$GLOBALS['ecs']->table('bearer_info')."AS bi WHERE oi.shipping_status = 3 AND oi.order_id = ot.order_id AND ot.bearer_id = bi.id");
    return $order_status;
}

//查看所有的持票人信息
function get_order_bearer_info($order){
    $sql = "SELECT bi.* FROM ".$GLOBALS['ecs']->table('order_ticket')."AS ot," .$GLOBALS['ecs']->table('bearer_info')."AS bi WHERE ot.bearer_id = bi.id AND ot.order_id = '".$order['order_id']."'";
    $bearer_info_list = $GLOBALS['db']->getAll($sql);
    return $bearer_info_list;
}
//统计订单套餐中所有的票
function get_combo_num($order_id){
    //统计订单中的球票
    $sql = "SELECT DISTINCT goods_id FROM ".$GLOBALS['ecs']->table('order_info')." AS oi,".$GLOBALS['ecs']->table('order_combo')." AS oc WHERE oi.order_id=oc.order_id AND oi.order_id=$order_id";
    $combo_goods_id = $GLOBALS['db']->getAll($sql);
    if($combo_goods_id) {
        foreach ($combo_goods_id as $key => $val) {
            //查找这个订单中这张球票买了几张
            $sql2 = "SELECT * FROM " . $GLOBALS['ecs']->table('order_combo') . " AS ot WHERE ot.goods_id=$val[goods_id] and order_id=$order_id";
            $combo_goods_num = count($GLOBALS['db']->getAll($sql2));
            //查找订单中这张票卖了多少张
            $sql1 = "SELECT * FROM " . $GLOBALS['ecs']->table('order_combo') . " AS oc WHERE oc.goods_id=$val[goods_id]";
            $combo_goods_id[$key]['goods_num'] = count($GLOBALS['db']->getAll($sql1)) - $combo_goods_num;
            $combo_goods_id[$key]['combo_goods_num'] = $combo_goods_num;
        }
    }
    return $combo_goods_id;
}
//统计订单中普票数量
function get_ticket_num($order_id){
    //统计订单中的普通球票
    $sql = "SELECT DISTINCT goods_id FROM ".$GLOBALS['ecs']->table('order_info')." AS oi,".$GLOBALS['ecs']->table('order_ticket')." AS ot WHERE oi.order_id=ot.order_id AND oi.order_id=$order_id";
    $ticket_goods_id = $GLOBALS['db']->getAll($sql);
    if($ticket_goods_id) {
        foreach ($ticket_goods_id as $key => $val) {
            //查找这个订单中这张球票买了几张
            $sql2 = "SELECT * FROM " . $GLOBALS['ecs']->table('order_ticket') . " AS ot WHERE ot.goods_id=$val[goods_id] and order_id=$order_id";
            $ticket_goods_num = count($GLOBALS['db']->getAll($sql2));
            //查找订单中这张票卖了多少张
            $sql1 = "SELECT * FROM " . $GLOBALS['ecs']->table('order_ticket') . " AS ot WHERE ot.goods_id=$val[goods_id]";
            $ticket_goods_id[$key]['goods_num'] = count($GLOBALS['db']->getAll($sql1)) - $ticket_goods_num;
            $ticket_goods_id[$key]['ticket_goods_num'] = $ticket_goods_num;
        }
    }
    return $ticket_goods_id;
}
//获取批次
function get_goods_batch($id){
    $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('batch_info')." WHERE goods_id = $id";
    return $GLOBALS['db']->getAll($sql);
}

//订单持票人分页
function bearer_info_page($order_id){
    /* 记录总数 */
    $sql = "select b.* from sk_order_ticket t,sk_bearer_info b where t.order_id=$order_id AND b.id=t.bearer_id union all select b.* from sk_order_combo c,sk_bearer_info b where c.order_id=$order_id AND b.id=c.bearer_id";
    $filter['record_count'] = count($GLOBALS['db']->getAll($sql));

    /* 分页大小 */
    $filter = page_and_size($filter);
    $sql = "select b.* from sk_order_ticket t,sk_bearer_info b where t.order_id=$order_id AND b.id=t.bearer_id union all select b.* from sk_order_combo c,sk_bearer_info b where c.order_id=$order_id AND b.id=c.bearer_id LIMIT " . $filter['start'] . ",$filter[page_size]";
    $row = $GLOBALS['db']->getAll($sql);
    return array('bearer_info' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
}

//订单持票人信息
function bearer_info_id($bearer_id,$order_id,$goods_id){
    $sql = "SELECT b.*,ot.ticket_code FROM ".$GLOBALS['ecs']->table('order_ticket')." AS ot,".$GLOBALS['ecs']->table('bearer_info')." AS b WHERE ot.order_id=$order_id AND ot.bearer_id=$bearer_id AND ot.bearer_id=b.id AND ot.goods_id=$goods_id";
    $ticket = $GLOBALS['db']->getRow($sql);
    if($ticket){
        return $ticket;
    }else {
        $sql = "SELECT b.*,oc.ticket_code FROM " . $GLOBALS['ecs']->table('order_combo') . " AS oc," . $GLOBALS['ecs']->table('bearer_info') . " AS b WHERE oc.order_id=$order_id AND oc.bearer_id=$bearer_id AND oc.bearer_id=b.id AND oc.goods_id=$goods_id";
        $combo = $GLOBALS['db']->getRow($sql);
        return $combo;
    }
}
//查询普通球票票号
function get_ticket_code($order_id){
    $sql = "SELECT goods_id,ticket_code FROM ".$GLOBALS['ecs']->table('order_ticket')." WHERE order_id=$order_id";
    return $GLOBALS['db']->getAll($sql);
}
//查询套餐球票票号
function get_combo_code($order_id){
    $sql = "SELECT goods_id,ticket_code FROM ".$GLOBALS['ecs']->table('order_combo')." WHERE order_id=$order_id";
    return $GLOBALS['db']->getAll($sql);
}