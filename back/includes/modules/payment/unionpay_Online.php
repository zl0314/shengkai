...
// 包含配置文件
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/unionpay_Online.php';
 ...
 * Class unionpay
 */
class unionpay_Online
{
    public function  unionpay_Online()
    {
    ...
    public function ConsumeUndo($order)
    {
        $params = array(
            'version' => '5.0.0',            //版本?
            'encoding' => 'utf-8',            //编码方式
            ...
            'accessType' => '0',            //接入类型
            'channelType' => '07',            //渠道类型
            'orderId' => date('YmdHis'),    //商户订单号，重新产生，不同于原消?
            'merId' => '898111459410116',            //商户代码，请改成自己的测试商户号
            'origQryId' => $order['queryid'],    //原消费的queryId，可以从查询接口或????知接口中获?
            'txnTime' => date('YmdHis'),    //订单发??时间，重新产生，不同于原消?
            'txnAmt' => $order['txnAmt'],              //交易金额，消费撤?时需和原消费??
            'backUrl' => SDK_BACK_NOTIFY_URL,       //后台通知地址
            'reqReserved' => 'unionpay_Online', //请求方保留域，??传字段，查询????知、对账文件中均会原样出现
        );
        Logger::DEBUG("ConsumeUndo:". json_encode($params));
// 签名
        sign($params);
// 发??信息到后台
        $result = sendHttpRequest($params, SDK_BACK_TRANS_URL);
        Logger::DEBUG("ConsumeUndo_result:". json_encode($result));
    }
    public function respond()
    {
        Logger::DEBUG("respond entry");
        __load("OrderService");
        Logger::DEBUG("load OrderService");
        $order_obj = new OrderService();
        Logger::DEBUG("new OrderService");
                ...
                if ($order_info['order_amount'] * 100 == $_POST['txnAmt']) {
                    ...
                    if ($order_info['pay_status'] == "0") {
                        ...
                        if(substr($order_info['mobile'],0,2) == '86'){
                            ...
                            Logger::DEBUG("unionpay_Online send:". $order_info['mobile']);
                        }else{
                            ...
                            Logger::DEBUG("unionpay_Online English send:".$order_info['mobile']);
                        }
...
//                        }
                ...
                Logger::DEBUG("Order verify failed.");
            }
        } else {
            Logger::DEBUG("Order info is null.");
        }
