...
class integrate
{
    ...
    function login($username, $password, $remember )
    {
        ...
        if ($this->check_user($username, $password) > 0)
        {
            $sql = "SELECT " . $this->field_name .
               ...
               " WHERE " . $this->field_phone . " = '$username' or " . $this->field_name . " = '$username' ";
    ...
    function logout ()
    {
         $this->set_cookie($user_name, null); //清除cookie
...
*/
    ...
    function edit_user($cfg)
    {
        ...
        if (isset($cfg['type']) && $this->field_type != 'NULL'){
            $values[] = $this->field_type . "='" . $cfg['type'] . "'";
        }
    ...
    {
        $post_mobile = $mobile;
        $sql = 'SELECT COUNT(mobile_phone) FROM ' . $this->table('users')." where mobile_phone=".$post_mobile;
        return $this->db->getOne($sql);
         
    }
    
    //判断输入的验证码是否正确
    function check_is_verifycode($is_verifycode,$mobile_phone)
    {
        $sql = 'SELECT verifycode FROM ' . $this->table('verifycode')." where verifycode=$is_verifycode AND mobile=$mobile_phone";
        return $this->db->getOne($sql);
         
    }

    //更新密码
    function check_is_tihuan($mobile_phone,$new_mm)
    {
//        //获取ec_salt的加密数据
//        $sql = "SELECT ec_salt from sk_users WHERE mobile_phone =$mobile_phone";
//        $res=$this->db->getOne($sql);
//        //加密过后存入数据库
//        $mm=md5(md5($new_mm).$res);
//        $sql = "UPDATE sk_users SET password='".$mm."'  WHERE mobile_phone =$mobile_phone";
//        $this->db->getAll($sql);
//        //查询是否插入成功
//        $sql = "SELECT COUNT(password) from sk_users WHERE mobile_phone =$mobile_phone";
//        return $this->db->getOne($sql);
        $sql = "SELECT user_name from sk_users WHERE mobile_phone =$mobile_phone";
        $res=$this->db->getOne($sql);
        if(empty($res)){
            return false;
        }else{
            return $this->edit_user(array("username"=>$res,'password'=>$new_mm));
        }
...
//    }
     ...
     * @return void
     */
    function set_cookie($username='', $remember )
    {    
        if (empty($username))
        {
            /* 摧毁cookie */
            $time = time() - 3600;
            setcookie("username",  '', $time, $this->cookie_path);            
            setcookie("password", '', $time, $this->cookie_path);
        ...
        {   
            ...
            if ($row)
            {
                ...
                setcookie("username", $username, $time, $this->cookie_path, $this->cookie_domain);
                setcookie("password", $row['password'], $time, $this->cookie_path, $this->cookie_domain);
            }
        }
    }
