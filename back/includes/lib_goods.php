...
{
    $nowTime = time();
    $sql = "SELECT a.goods_id, b.num_start FROM sk_goods AS a " . 
            " LEFT JOIN sk_number AS b ON b.id = a.number_id " .
            " WHERE a.is_on_sale = 1 AND is_ticket = 1";
    $goods_list = $GLOBALS['db']->getAll($sql);
    foreach($goods_list as $val){
        $diff = floor((strtotime($val['num_start'])-$nowTime)/86400);
        if($diff < 3){
            $GLOBALS['db']->getRow("UPDATE sk_goods SET is_on_sale = 0 WHERE goods_id = ".$val['goods_id']);
        }
    }
    return true;
}
