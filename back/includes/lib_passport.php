 ...
 * @return  bool         $bool
 */
function register($username, $password, $email, $other = array(),$is_third,$in_urt)
{
    /* 检查注册是否关闭 */
...
//    }
    ...
    if (!$GLOBALS['user']->add_user($username, $password, $email))
    {
        ...
        {
            $GLOBALS['err']->add(sprintf("用户名已存在", $username));
        }
    ...
    else
    {
        ...
        if($is_third==1){
        $GLOBALS['db']->query( 'UPDATE '. $GLOBALS['ecs']->table('users') . ' SET is_third = ' . $is_third . ',in_urt = "' . $in_urt .' " WHERE user_id = ' . $_SESSION['user_id']);
        }
