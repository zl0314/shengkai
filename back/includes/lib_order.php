...
SELECT
	(
		ht.goods_price * ht.goods_number
	) AS gp,
	ht.goods_price,
	ht.room_type,
	ht.people,
...
, sk_order_hotel AS ht 
WHERE
ot.goods_id = g.goods_id AND g.number_id = n.id AND p.id = n.pitch_id AND p.region_id = r.region_id
AND n.color_id = c.color_id AND ht.order_id = ot.order_id AND h.id=ht.hotel_id AND ot.order_id = {$order_id} GROUP BY ht.hotel_start_date";
...
{
    $sql = "select Sum(goods_price*goods_number) from sk_order_hotel where order_id=" . $order_id;
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}
...
{
    $sql = "SELECT
rg.region_name as re1,rg2.region_name as re2,op.fly_date,op.goods_number,op.goods_price,(op.goods_number*op.goods_price) as planesum
FROM 
