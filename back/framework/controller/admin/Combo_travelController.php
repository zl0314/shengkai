...
class Combo_travelController extends Controller
{
    ...
    {
        $combo_travel_type_id = intval($_GET['combo_travel_type_id']);
        $combo_travel_type = $this->combo_travel_type->get_Combo_travel_type($combo_travel_type_id);
        $combo_travel_list = $this->combo_travel->get_All_Combo_travel_by_type_id($combo_travel_type_id);

        $this->assign('action_link', array('text' => "添加套餐行程", 'href' => 'route.php?con=combo_travel&act=add&combo_travel_type_id=' . $combo_travel_type_id));
        $this->assign('combo_travel_list', $combo_travel_list['list']);
        $this->assign('ur_here', "套餐行程{$combo_travel_type['type_name']}列表");
    ...
    public function add()
    {
        ...
        $this->assign('action_link', array('text' => "套餐列表", 'href' => 'route.php?con=combo_travel'));
        //获取套餐行程信息
    ...
    public function edit()
    {
        ...
        if ($act == "add") {
            ...
            $combo_travel_type_id = $_POST['combo_travel_type_id'];
            $link = array(array('text' => "套餐行程列表", 'href' => 'route.php?con=combo_travel&combo_travel_type_id=' . $combo_travel_type_id), array('text' => "继续添加行程", 'href' => 'route.php?con=combo_travel&act=add&combo_travel_type_id=' . $combo_travel_type_id));
            if (empty($res)) {
                sys_msg("添加失败", 0, $link);
            } else {
            ...
            $combo_travel_type_id = $res['combo_travel_type_id'];
            $link = array(array('text' => "套餐行程列表", 'href' => 'route.php?con=combo_travel&combo_travel_type_id=' . $combo_travel_type_id));
            if ($res) {
    ...
    public function update()
    {
        ...
        $link = array('text' => "行程列表", 'href' => 'route.php?con=combo_travel&combo_travel_type_id=' . $combo_travel['combo_travel_type_id']);
        $this->assign('action_link', $link);
