...
if ($_REQUEST['step'] == 'cart')
{
	...
	if ($_CFG['ecsdxt_mobile_cons'] == '1' && (!$row || !ismobile($row['mobile_phone'])))
	{
		...
		show_message($_LANG['ecsdxt_mobile_cons_notice'], '绑定手机号', 'user.php?act=bindmobile', 'info');
	}
}
...
//-- 添加商品到购物车
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'add_to_cart_game')
{
    __load("CartService");
...
if ($_REQUEST['step'] == 'add_to_cart_game_pc')
{
    __load("CartService");
    $cart_obj = new CartService();
    include_once('includes/cls_json.php');
...
if ($_REQUEST['step'] == 'add_to_cart_games')
{
    __load("CartService");
    $cart_obj = new CartService();
    include_once('includes/cls_json.php');
...
if ($_REQUEST['step'] == "get_room_type"){
    //获取房间类型
    $room_type=get_room_infoadd_to_cart_game($_POST['id']);
    $room_info=array();
    foreach($room_type AS $key=>$value){
        $room_info[$key]=$value;
        $room_info[$key]['hotel_id']=$_POST['id'];
        $room_info[$key]['rec_id']=$_POST['rec_id'];
    }
    $json  = new JSON; 
    exit($json->encode($room_info)); 
}
 if ($_REQUEST['step'] == "add_to_cart_hotel"){

    __load("GoodsService");
    $goods_obj=new GoodsService();
    if($_POST['hotel_id']){
    //根据酒店id获取房间信息
    $room_info=get_room_info($_POST['hotel_id']); 
    $hotel_name=get_hotel_name($_POST['hotel_id']);
    $arr=array();
        $temp_start_date=$value['id'].'_start_date';
        $temp_end_date=$value['id'].'_end_date';
        $temp_num=$value['id'].'_num';
        $arr['hotel_start_date']=$_POST[$temp_start_date];
        $arr['hotel_end_date']=$_POST[$temp_end_date];
        $arr['room_num']=$_POST[$temp_num];
//        $arr['room_money']=$value['room_money'];
        $arr['room_type']=$value['type_name'];
        $arr['session_id']=SESS_ID;
        $arr['user_id']=$_SESSION['user_id'];
//        $arr['hotel_id']=$_POST['hotel_id']; 
        $arr['hotel_rec_id']=$goods_obj->add_hotel($arr);
//        $arr['hotel_name']=$hotel_name; 
//        $arr['tick_id']=$_POST['rec_id']; 
//        $arr['type_name']=$_POST['type_name'];
            $json  = new JSON; 
            exit($json->encode($_POST)); 
              
    }else{
        show_message(添加失败, '', '', 'warning');
    }
 }
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
	...
	/* 客户下订单时给客户发送短信提醒 */
	if ($_CFG['ecsdxt_customer_placed'] == '1' && $row['mobile_phone'] != '')
	{
		require_once(ROOT_PATH . 'includes/lib_sms.php');
...
function get_room_info($id){
    $sql="SELECT rt.type_name,rm.* FROM ". $GLOBALS['ecs']->table('room') ."AS rm,". $GLOBALS['ecs']->table('room_type') ."AS rt"." WHERE rm.room_type=rt.id AND rm.hotel_id=$id GROUP BY rt.type_name";
    return $GLOBALS['db']->getAll($sql);
}
//通过酒店id获取酒店名称
function get_hotel_name($id){
    return $GLOBALS['db']->getOne("SELECT hotel_name FROM". $GLOBALS['ecs']->table('hotel')."WHERE id=$id");
}
//显示用户选择的酒店信息
function get_hotel_show($res){
    $sql="SELECT c.*,rt.type_name FROM". $GLOBALS['ecs']->table('cart') ."AS c,". $GLOBALS['ecs']->table('room_type') ."AS rt"." WHERE c.rec_id=$res AND c.room_type=rt.id";
    return $GLOBALS['db']->getAll($sql);;
